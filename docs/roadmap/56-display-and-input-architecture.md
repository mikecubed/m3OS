...
> **Phase 68 closure note (planned):** Five concrete completion gaps that survived the Phase 56 close are tracked in [Phase 68 — Display Server Closeout](./68-display-server-closeout.md): (1) the four `publish_*` functions in `userspace/display_server/src/control.rs` carry `TODO(subscription-push)` markers and never transmit subscribed events over the wire — Phase 68 adds `flush_subscriber_ring` and two new `ControlEvent` kinds (`LayerEvent`, `CursorEvent`); (2) the compositor at `userspace/display_server/src/compose.rs:164-175` repaints the entire framebuffer on cursor-only frames — Phase 68 adds a `DamageTracker` and clips blits to the dirty union; (3) `KeyEvent` carries no L/R modifier discrimination — Phase 68 adds a `ModifierSide` field and bumps the global `PROTOCOL_VERSION` from `1` to `2` with a version-1 client compatibility shim; (4) `mouse_server` exists as an unmanaged binary — Phase 68 adds `kernel/initrd/etc/services.d/mouse_server.conf` declaring `depends=kbd_server` and registers it under init's supervision; (5) the supervisor cannot dispatch a typed action when a service exhausts its restart budget — Phase 68 extracts `userspace/init/src/manifest.rs` + `supervisor.rs` from `main.rs` and adds a typed `on-restart=` directive (`LogAndContinue` / `TextFallback` / `Panic`). This note will be updated to "(closed)" when Phase 68 ships.

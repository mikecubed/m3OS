...
- Tiling layout algorithms (dwindle, master-stack, BSP, manual), workspace state machines, keybind chord engine, and `hyprctl`-style scripting surface beyond the minimum control-socket verbs — **delivered in [Phase 72 — Compositor: Multi-Toplevel, Tiling Layout, and Workspaces](./72-compositor-tiling-workspaces.md)**. The new `userspace/lib/layout/` crate hosts `LayoutPolicy` implementations (master-stack, dwindle, spiral, grid, tabbed, fullscreen); `display_server` adds a per-output `WorkspaceManager` with nine numbered workspaces, a `BindStack` chord engine with the canonical `SUPER+1..9` / `SUPER+SHIFT+1..9` / `SUPER+TAB` / `SUPER+RETURN` set plus resize mode, per-window borders + outer/inner gaps painted after the compose pass, an `/etc/compositor.conf` TOML parser with `m3ctl reload`, and eight new `ControlCommand` verbs (`SetLayout`, `SwitchWorkspace`, `MoveToWorkspace`, `Reload`, `QueryWindows`, `QueryWorkspaces`, `SetMasterRatio`, `TileFullscreen`).
- Native bar / launcher / notification daemon / lockscreen client implementations — ~~Phase 57b territory~~ **✅ delivered in [Phase 73 — Compositor: Polish](./73-compositor-polish.md)** (`userspace/{bar,launcher,notifyd,lockscreen}`).
- Animation engine (timing curves, vblank-aligned scheduling, window-move/fade/workspace-slide animations) — ~~Phase 57c territory~~ **✅ delivered in [Phase 73 — Compositor: Polish](./73-compositor-polish.md)** (`userspace/display_server/src/animation.rs`).
...
> **Phase 68 closure note:** Five concrete completion gaps that survived the Phase 56 close are closed in [Phase 68 — Display Server Closeout](./68-display-server-closeout.md): (1) `flush_subscriber_ring` lives in `kernel_core::display::subscription` and the four `publish_*` helpers in `userspace/display_server/src/control.rs` call it after each enqueue (the `TODO(subscription-push)` markers are gone); two new `ControlEvent` kinds — `LayerEvent` and `CursorEvent` — extend the subscribable kind space. (2) `DamageTracker` (`kernel_core::display::damage`) tracks per-frame dirty rectangles and the cursor-only fast path in `compose.rs` skips the surface compose pass entirely when only the cursor moved, blitting strictly fewer pixels than the framebuffer resolution. (3) `KeyEvent` now carries a `modifier_side: ModifierSide` field with `Left` / `Right` / `Either` variants, the global `PROTOCOL_VERSION` bumped from `1` to `2`, and `KeyEvent::encode_v1` / `decode_v1` + `downgrade_for_v1_client` form the version-1 compatibility shim. (4) `kernel/initrd/etc/services.d/mouse_server.conf` exists statically with `depends=kbd_server` and the binary is registered in init's `KNOWN_CONFIGS`. (5) Supervisor primitives in `kernel_core::init::supervisor` (`start_services_ordered`, `handle_budget_exhaustion`) plus the typed `OnRestartAction` (`LogAndContinue` / `TextFallback` / `Panic`) in `kernel_core::init::manifest` are re-exported by `userspace/init/src/manifest.rs` and `supervisor.rs`. See `docs/roadmap/68-display-server-closeout.md` for the implementation summary.

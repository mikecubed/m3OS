...
- **A dynamic interpreter** — a real musl `libc.so` + the syscall coverage a dynamic libc needs, then a dynamic `python3` with real `lib-dynload` `.so` extensions and `ctypes`/`dlopen` of arbitrary shared objects — is [Phase 93 (Dynamic C Runtime)](./93-dynamic-c-runtime.md). 85c ships fully static (every C extension builtin) because m3OS's Phase 76 loader has no `libc.so` to bind a dynamic C program against; Phase 93 lifts that.

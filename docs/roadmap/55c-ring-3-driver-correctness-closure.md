...
- **Many-to-one binding** (multiple notifications bound to one TCB). The e1000 case needs exactly one; display/input/audio cases in Phase 56/57 can still be modeled with one binding per driver. A future phase can lift this if a driver is introduced that genuinely needs two independent async sources. **Phase 74 update:** `sys_notif_bind` (syscall `0x1111`) is in place and the userspace `notif_bind` wrapper ships in `syscall-lib`; the kernel binding is task-scope rather than endpoint-scope so a driver that wants to multiplex two notifications onto one recv loop binds each notification to the driver task once and the recv-wake path drains whichever fires.
- **Timed recv** (`ipc_recv_timeout`). The e1000 deadlock goes away without it. Phase 55c ships only the bound-notification path; any timeout support is a separate design decision. **Phase 74 update:** delivered as `sys_ipc_recv_timeout` (syscall `0x111A`) alongside `sys_ipc_call_timeout` (`0x1119`); both use the existing Phase 57a `block_current_until` deadline path with race-free endpoint-queue cleanup on deadline expiry.

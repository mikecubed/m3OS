...
**Status:** In progress — **Tracks A, B, and C landed.** A: `cargo xtask debug` (QEMU gdbstub + `kdebug` DWARF build). B: `#DB`/`#BP` dispatch + single-step + `DR0`–`DR7` + `int3` patch (`debug-substrate-smoke` PASS). C: the in-kernel `kgdb` stub — full-GPR `#BP`/`#DB` naked entry, polled COM2 transport, the RSP command loop (`kernel_core::gdb_rsp` codec), SMP all-stop + panic hook, all behind the `kgdb` feature (`kgdb-smoke` PASS: raw-RSP attach → breakpoint hit → register/memory read-back → all-stop sentinel). **Track D in progress:** D.1 + D.2 landed — the `ptrace` substrate (traced-process stop/notify, `SIGTRAP` on ring-3 `int3`, `sys_ptrace` with TRACEME/CONT/SINGLESTEP/GETREGS/SETREGS/PEEK+POKETEXT/DETACH, cross-address-space peek/poke) behind the `ptrace` feature, validated by a native tracer/tracee (`ptrace-smoke` PASS). D.3 (`m3gdbserver` RSP↔ptrace) + D.4 (userspace symbol retention) next.

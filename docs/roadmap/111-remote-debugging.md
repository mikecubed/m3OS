...
**Status:** ✅ **Complete (merged).** Tracks A–D all landed and merged to `main` (PRs #311/#312/#313/#314/#315); every QEMU-validatable gate PASS. The only follow-on is a source-level DWARF userspace build for a *real* host `gdb` (the dev machine has none; every arm here is validated with raw-RSP clients). A: `cargo xtask debug` (QEMU gdbstub + `kdebug` DWARF build). B: `#DB`/`#BP` dispatch + single-step + `DR0`–`DR7` + `int3` patch (`debug-substrate-smoke` PASS). C: the in-kernel `kgdb` stub — full-GPR `#BP`/`#DB` naked entry, polled COM2 transport, the RSP command loop (`kernel_core::gdb_rsp` codec), SMP all-stop + panic hook, all behind the `kgdb` feature (`kgdb-smoke` PASS: raw-RSP attach → breakpoint hit → register/memory read-back → all-stop sentinel). **Track D landed** (behind the `ptrace` feature): D.1 the traced-process stop/notify substrate (`SIGTRAP` on ring-3 `int3`, cross-address-space peek/poke), D.2 `sys_ptrace` (TRACEME/CONT/SINGLESTEP/GETREGS/SETREGS/PEEK+POKETEXT/DETACH), and D.3 `m3gdbserver` — a native gdbserver that forks + `TRACEME` + `execve`s a tracee (which **exec-stops**) and translates GDB RSP ↔ `sys_ptrace` over TCP, so a host GDB-RSP client debugs a ring-3 program remotely. Validated by `ptrace-smoke` (native tracer/tracee) + `ptrace-gdbserver-smoke` (host RSP client over SLIRP hostfwd, incl. single-step + breakpoint) — both PASS. D.4: the native userspace ELFs keep their symbol table host-side (symbol-level `gdb` works today); a source-level DWARF build variant is the one operator follow-on.

**Real-`gdb` workflow (operator, needs a host `gdb`):** in the guest, `m3gdbserver <port> <program>`; on the host, `gdb target/x86_64-m3os/release/<program>` then `target remote 127.0.0.1:<forwarded-port>` (a `cargo xtask run` forwards `2222→22`/`2323→23`; forward a gdbserver port by adding a `hostfwd` rule). Symbol-level (`break <fn>`, named `bt`) works against the release ELF; source-level `list`/`step` needs a `-g` build of `<program>`.

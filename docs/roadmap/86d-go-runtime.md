...
#### As-built decision (86d): path (b) — `asyncpreemptoff`, no IRQ-return delivery

**Chosen: path (b).** The Go runtime is run with `GODEBUG=asyncpreemptoff=1`, and the kernel's timer-IRQ-return path (`timer_handler_user`, `interrupts.rs:1587`) is left **unchanged** — it does scheduling/preemption but never calls `check_pending_signals`. `SIGURG`/`tgkill` are still wired (so the syscalls don't `ENOSYS` and `SIGURG` is deliverable **at syscall-return**, which is where Go's signal handler runs after `rt_sigaction(SIGURG, …)`).

**Rationale.** Building a signal frame from interrupt context (the IRQ path) is a genuine destabilization risk: the existing signal machinery only ever constructs a frame at syscall-return, and doing it from a timer ISR would have to reconstruct the interrupted user `RIP`/`RSP`/register state from the preempt trap frame and contend with nested-interrupt/SMAP/`preempt_count` invariants. For a *first* Go bring-up whose acceptance is a goroutine **channel rendezvous** + a **plaintext HTTP GET**, true async preemption of a compute-bound goroutine is not exercised: both workloads block on the kernel frequently (futex/channel, epoll/network, `read`), so any `SIGURG` Go sends lands at the next syscall-return, and GC stop-the-world reaches its safepoints cooperatively. `asyncpreemptoff=1` additionally tells the runtime **not to rely** on signal-based preemption (it stops *sending* `SIGURG` for preemption), so a compute-bound goroutine with no function-call safepoints simply won't be async-preempted — the documented limitation. The IRQ-return delivery path is deferred to a later phase (recorded under "Deferred Until Later").

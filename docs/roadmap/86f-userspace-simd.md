...
**Status:** Done ✅ — userspace builds hardware-float SSE/SSE2+AES on `x86_64-m3os.json` (kernel soft-float unchanged); signal-frame FPU save/restore landed with a sanitized-XSAVE-header sigreturn; `_start` entry stack fixed to the psABI 16-byte alignment (18 naked-trampoline conversions); AES-NI measured ≈27× over forced-soft AES-CTR (host A/B); kernel at `0.86.5`. Deviation: `ld-musl` stays on `x86_64-unknown-none` (must remain PIE/`ET_DYN`).

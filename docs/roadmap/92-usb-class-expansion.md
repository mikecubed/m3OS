...
**Status:** Complete (92a–92e landed; kernel `0.92.5`) — two items explicitly deferred with stated blockers (live UAS command/data driving; mount-teardown-on-detach), see *Deferred Until Later*
**Source Ref:** phase-92
...
- **Mount-teardown-on-detach** (Track C.4, the `/mnt/usb<n>` half) — server-side detach + Disable-Slot reclamation (C.1–C.3 / H.3) and per-driver state release are live and gated, and the kernel `unmount_usb(prefix)` teardown (`kernel/src/fs/ext2.rs`) is implemented and ready to call, but the resident `usb-storage` block-server loop cannot yet interleave a detach notification with a block request — that needs a **non-blocking IPC recv** primitive (`ipc_try_recv_msg`) that does not exist in `syscall-lib`. Until that lands, unplugging a *mounted* stick leaves the mount stale (it does not wedge the VFS); detach of an *unmounted* stick is fully handled.
- **Multi-stick concurrent mounts** (Track D.4, `/mnt/usb1`+) — the kernel `blk::remote` registry holds up to `MAX_REMOTE_BLOCK`=4 devices and `/mnt/usb1` is pre-created + routable, but the `usb-storage` daemon is single-device per process (hardcoded `usb0.block`); a second concurrent stick needs the daemon's service name + target device parameterized (a second instance) before a two-stick CI gate is meaningful.

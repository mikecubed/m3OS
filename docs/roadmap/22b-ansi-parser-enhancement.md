...
- DEC private mode set/reset for any code other than `?25` (DECTCEM) — *closed in Phase 69 (`ConsoleCmd::DecPrivateMode { codes: [u16; MAX_PARAMS], count, set }` covers `?1049`, `?47`, `?9`, `?1000`, `?1006`, `?2004`; a single CSI may carry multiple semicolon-separated codes such as `?1006;1000h` from the terminfo `XM` capability and consumers iterate `codes[..count]`; consumers that do not recognise a code drop it silently).*

...
- An inode-identity battery passes on the ext2/`vfs_server` root (the Phase 88 `st_ino` rigor): `ln` a hardlink, then `stat -c %h` reports `nlink` 2 then 1 across an `rm` of the original name (whose content survives via the link), and `stat -c %i … | sort -u | wc -l` reports `1` for the two hardlinked names (shared inode) versus `2` for two distinct files (distinct non-zero inodes — the regression guard against a `st_ino=0` collapse).
...
- **`stat`/inode-identity rigor.** uutils is stricter about `(st_dev, st_ino)` than the hand-built tools (e.g. `ls -i`, hardlink detection in `du`/`cp`). The prerequisite — a canonical `fill_stat()` giving VFS/ext2 files consistent, non-zero `(st_dev, st_ino)` — landed in **Phase 88** (PR #240; before this phase), so this is **no longer deferred**: `coreutils-smoke` now exercises it directly on the ext2/`vfs_server` root (`ln` a hardlink → `stat -c %h` reports nlink 2 then 1 across an unlink; `stat -c %i … | sort -u | wc -l` confirms hardlinked names share one inode while distinct files have distinct non-zero inodes — the regression guard against a `st_ino=0` collapse). Deeper locale/`-i`-format edge cases remain best-effort.

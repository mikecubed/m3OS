...
A JIT engine (future Node.js in Phase 89, Cranelift, etc.) cannot use a page that is simultaneously `PROT_WRITE | PROT_EXEC`. The supported pattern is: (1) allocate with `mmap(PROT_WRITE)`, (2) write machine code, (3) `mprotect(PROT_READ | PROT_EXEC)`. This is documented in `docs/appendix/architecture-and-syscalls.md` as the required JIT code-generation pattern. No new syscall is needed; the pattern works with existing `mmap` and `mprotect`.

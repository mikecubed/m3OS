...
**Status:** Implemented — network-free acceptance verified on m3OS; the host-key-**mismatch reject** is now verified **live** against `github.com:22` (the `M3OS_GIT_SSH_NET=1` step hits dropbear's real `host key mismatch for` KEX-time reject), and the real `clone` remains credential/egress-gated (needs a GitHub-registered key), skipping-with-reason in unconfigured CI (the `tls-smoke`/`dns-smoke` PASS-vs-SKIP contract). The live SSH path's one remaining kernel blocker — **non-blocking `connect` (`EINPROGRESS`/poll-writability/`SO_ERROR`)** — was **implemented in this sub-phase** (≈140 LOC kernel in `sys_connect`/`fd_poll_events`/`getsockopt` + `net::reconcile_connecting`, proven by `connect-smoke` and adversarially reviewed); see "Deferred Until Later" for the implementation detail.
...
Build the chosen client as a static `.m3pkg` landing in `/usr/pkg/`, installable with `pkg install ssh`. **dropbear branch:** a `build_dropbear` port routed through the shared musl-toolchain plumbing, using dropbear's bundled `libtomcrypt`/`libtommath` (`--enable-bundled-libtom`) so the client links no system crypto library and stays dependency-free, exposing `dbclient -y/-i/-p` and built-in `known_hosts`/TOFU. **sunset branch:** a Rust client harness in `userspace/ssh/` driving `Runner::new_client` + `open_client_session` over the `async-rt` `Reactor`, surfacing `CliEvent::Hostkey` so the app does TOFU + file I/O. GitHub's host keys are seeded as rotatable on-disk **data** (`github.com` and `ssh.github.com` ed25519 entries), and a **mismatched host key is rejected** (mandatory negative test).
...
**dropbear:** `build_dropbear` in `xtask/src/port_build.rs` follows the AGENTS.md port rules — `musl_toolchain()` (`port_build.rs:111`) for `CC`/`AR`/`RANLIB`, `musl_extra_ldflags_joined()` (`port_build.rs:105`) for the static LDFLAGS (`-static` + `--disable-harden` force the static link), `--host=x86_64-linux-musl`, using dropbear's bundled `libtomcrypt`/`libtommath` (`--enable-bundled-libtom`) so the client links no system crypto library and stays dependency-free. **sunset:** a new `userspace/ssh/` binary linking `crypto-lib` (X25519/Ed25519/ChaCha20-Poly1305/SHA-2/HMAC, fed by the 86a CSPRNG) and driving the connection through `userspace/async-rt/src/reactor.rs:23`'s `Reactor`. Either way the artifact is a static binary on `PATH`, sealed into a `.m3pkg` and installed offline.
...
- **Non-blocking `connect` semantics — RESOLVED in this sub-phase (was THE one remaining kernel blocker for the live SSH path).** Originally scoped out of 86b, but landed here to ship a complete vertical slice rather than leave the live path half-done. The change (~140 LOC kernel, MEDIUM risk audited to APPROVE) makes `sys_connect` (`kernel/src/arch/x86_64/syscall/mod.rs`) honor the FD's non-blocking flag and return `EINPROGRESS` instead of busy-blocking; adds a `SocketState::Connecting` variant (`kernel/src/net/mod.rs`) lazily promoted to `Connected` by `net::reconcile_connecting` (the standing `net_task` drives the handshake in the background); adds a `getsockopt(SO_ERROR)` arm (was `ENOPROTOOPT`) deriving the connect result on demand; reports `POLLOUT` on connect-completion in the `poll`/`select` writability gate (`fd_poll_events`) and `POLLOUT|POLLERR` on failure; and reuses the existing `EALREADY`/`EISCONN` re-issue guard (the AF_UNIX path's pattern) before slot allocation so poll-driven clients cannot leak TCP slots. The blocking path is unchanged except a slot-leak fix on its `EINTR` arm surfaced by the audit. **Verified:** `connect-smoke` (egress-free: `EINPROGRESS` + `poll(POLLOUT)`-not-ready + `SO_ERROR=0` + `EALREADY`, runs in the main smoke flow) **and** the live `M3OS_GIT_SSH_NET=1` mismatch-reject, which now **auto-upgraded** from the previously-asserted `Connect failed` blocker to dropbear's **real** `host key mismatch for` KEX-time reject against `github.com:22` (over SLIRP, `+rdrand,+rdseed` credited for dropbear's blocking `getrandom()`, `git@github.com` single-quoted past `ion`'s `@`-array expansion) — the bad key is rejected at KEX and left on disk (not auto-accepted). The real `clone` still needs a GitHub-registered key (`M3OS_GIT_SSH_KEY`).
- TCP out-of-order / duplicate reassembly hardening (`kernel/src/net/tcp.rs` has no reassembly queue — it dup-ACKs and drops out-of-order segments) — general networking hardening, a lossy-SLIRP packfile-clone risk that is **not** on the mismatch-reject path (which rejects at KEX before any bulk transfer). Mitigated for the clone by `--depth 1 --single-branch` on a tiny repo + a clang-class timeout; a robust fix is its own kernel-TCP task, independent of the non-blocking-connect blocker above.

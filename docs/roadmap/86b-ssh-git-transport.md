...
**Status:** Implemented — network-free acceptance verified on m3OS; the live-GitHub items (real `ssh -T` / `clone` and the host-key-mismatch reject) are implemented and credential/egress-gated, skipping-with-reason in unconfigured CI (the `tls-smoke`/`dns-smoke` PASS-vs-SKIP contract). The live SSH path's one remaining kernel blocker (non-blocking `connect`) is a tracked cross-phase follow-up — see "Deferred Until Later".
...
- **Non-blocking `connect` semantics — THE one remaining kernel blocker for the live SSH path (tracked cross-phase follow-up).** This is a cross-cutting change to core TCP socket semantics, deliberately *out of 86b's scope* (this sub-phase ships **zero kernel diff** — only `ports/`, `xtask/`, `docs/`): `sys_connect` (`kernel/src/arch/x86_64/syscall/mod.rs`) must honor `O_NONBLOCK` and return `EINPROGRESS` instead of blocking; a new `SocketState::Connecting` must promote to `Connected` in the background; `getsockopt(SO_ERROR)` (today `ENOPROTOOPT`) must latch + clear the connect result; and the `poll`/`select` writability gate (`fd_poll_events`) must report `POLLOUT` on connect-completion (the AF_UNIX path already does the `EALREADY`/`EISCONN` half). It deserves its own design doc + acceptance + bare-metal/SLIRP validation (≈160 LOC, MEDIUM risk — it touches every existing blocking-connect caller, e.g. the telnet/ssh servers). **Empirically localized:** the `M3OS_GIT_SSH_NET=1` mismatch-reject was driven against `github.com:22` over QEMU SLIRP (with `+rdrand,+rdseed` credited so dropbear's blocking `getrandom()` reaches READY, and `git@github.com` single-quoted past `ion`'s `@`-array expansion). dropbear resolves the host and the **kernel TCP layer establishes the connection** (`connection established (active)`), but dropbear — which issues a **non-blocking** `connect()` then waits on socket writability — reports `Connect failed: unexpected failure` against m3OS's **synchronous** `sys_connect`. The SSH client, TOFU seed, and `GIT_SSH_COMMAND` wiring are otherwise verified, and the `M3OS_GIT_SSH_NET=1` smoke step now *positively asserts this localized blocker* (`Connect failed`), auto-upgrading to assert the real host-key reject (`host key mismatch for`) the moment this lands.

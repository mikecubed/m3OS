...
> **Verification status (post-implementation).** `[x]` = verified on m3OS or by `cargo xtask check`. Items tagged **[egress/cred-gated]** are implemented but cannot run without GitHub SSH egress + a registered key (the live-network half) — they SKIP-with-reason in this environment, exactly as `tls-smoke`/`dns-smoke` SKIP when their precondition is absent. Evidence: `cargo xtask git-ssh-smoke` → `PASSED core (22 steps in 150s)` — `pkg install ssh`/`git` OK, `dbclient -V`/`ssh -V` report `Dropbear v2024.86`, seeded GitHub ed25519 key round-trips the VFS; `cargo xtask port build dropbear` seals a static `.m3pkg` (rebuild = zero-compiler pkgcache hit); `cargo xtask check` clean at kernel **v0.86.1**.
...
- [ ] A **mismatched** host key is **REJECTED** (negative test asserts the clone aborts and the bad key is not written). **[egress-gated]** — implemented as the `M3OS_GIT_SSH_NET=1` step (plant wrong key → `ssh -T git@github.com` must print `Host key mismatch` and leave the bad entry unchanged); the host key is compared during KEX (before auth) so no registered key is needed, but it does need egress to `github.com:22`.
...
**Acceptance:**
- [ ] `M3OS_GIT_SSH_REGRESSION=1` boots m3OS, `pkg install`s `git` + `ssh`, and `git clone --depth 1 --single-branch ssh://git@github.com/<owner>/<repo>` succeeds — the packfile arrives and `HEAD` checks out (both serial-asserted). **[egress/cred-gated]** — the boot + `pkg install ssh`/`git` half is **verified** (smoke core, 22 steps); the live clone needs a GitHub-registered key + egress and runs under `M3OS_GIT_SSH_KEY=<key>` (+ `M3OS_GIT_SSH_NET=1`), asserting the packfile (`Receiving objects`) + a checked-out file from the cloned tree.

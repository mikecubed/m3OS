...
- [x] A **mismatched** host key is **REJECTED** (negative test asserts the connect aborts and the bad key is not written). **[verified live against `github.com:22`]** — the `M3OS_GIT_SSH_NET=1` step (plant wrong key → `ssh -T 'git@github.com'`) is a `WaitEither` over two outcomes: branch **a** `host key mismatch for` (dropbear's real KEX-time reject → then re-reads `known_hosts` and asserts the planted key SURVIVED) and branch **b** `Connect failed` (the localized non-blocking-connect blocker); a *silent accept* prints neither, so it fails the gate via timeout. Now that non-blocking `connect` is implemented (see Track A / design doc), the live run over SLIRP (entropy credited via `+rdrand,+rdseed`, ion `@`-expansion quoted) **matched branch a**: dropbear's non-blocking `connect()` establishes, reaches KEX, emits `host key mismatch for`, and the verify step confirms the bad key was left on disk. The step previously matched branch **b** (`Connect failed`) against the synchronous `sys_connect`; the auto-upgrade happened with no test edit.

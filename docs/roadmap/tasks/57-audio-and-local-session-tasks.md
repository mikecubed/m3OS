...
- [x] On `text-fallback`: `session_manager` stops the graphical services in reverse start order, releases the framebuffer back to the kernel console (the existing Phase 47 `restore_console` path), and surfaces an admin shell on the serial console. (real stop implementation in Phase 64 — Phase 57's `SupervisorBackend::stop` adapter logged the intent and returned `Ack` without sending a signal, so this rollback was logging-only; Phase 64's `lifecycle::stop_service` actually delivers SIGTERM/SIGKILL and observes the child's disappearance via a non-blocking `kill(pid, 0)` liveness probe — `session_manager` is not the parent of the supervised children so `sys_waitpid` is not available to it; rationale in `userspace/session_manager/src/runtime.rs`. `kernel_core::session::recover::execute_text_fallback_rollback` issues stops in reverse + invokes the `FramebufferRestorer`; userspace `session_manager::recover::run_text_fallback` wraps it with the syscall-backed `SyscallFramebufferRestorer`. The daemon stays alive after text-fallback so the serial admin shell stays available.)
...
- [x] Verbs: `session-state` (returns the current `SessionState`), `session-stop` (graceful shutdown, falls through to `text-fallback`), `session-restart` (graceful stop + start) (real stop motion ships in Phase 64 — Phase 57 routed all three to the stub backend that returned `Ack` unconditionally; Phase 64's `session-stop` / `session-restart` server-side handlers still call the F.4 text-fallback rollback, which now invokes the real `lifecycle::stop_service` motion via `InitSupervisorBackend::stop` for each child; per-service `restart_service` invocation through the control surface is a deferred follow-up. Phase 64 also adds a new `SessionStateDetailed` verb + `ServiceStates` reply carrying per-service `(name, ServiceState, restart_count, step_failures)` quads sourced from the table; the legacy `session-state` CLI continues to issue `ControlVerb::SessionState` and a CLI flag for the detailed variant is deferred.) (`kernel_core::session_control::ControlVerb` + `dispatch_authenticated`; userspace `session_manager::control::poll_control_once` wires them to the daemon's `ControlContext` + `SupervisorBackend`.)

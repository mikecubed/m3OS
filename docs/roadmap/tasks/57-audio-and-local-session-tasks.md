...
- [x] Verbs: `session-state` (returns the current `SessionState`), `session-stop` (graceful shutdown, falls through to `text-fallback`), `session-restart` (graceful stop + start) (real implementation in Phase 64 — Phase 57 routed all three to the Phase 57 stub backend that returned `Ack` unconditionally; Phase 64 routes `session-stop` / `session-restart` to `lifecycle::stop_service` / `lifecycle::restart_service` and the `session-state` reply carries per-service `(name, ServiceState, restart_count)` triples). (`kernel_core::session_control::ControlVerb` + `dispatch_authenticated`; userspace `session_manager::control::poll_control_once` wires them to the daemon's `ControlContext` + `SupervisorBackend`.)

...
|---|---|---|---|
| A | Widen `kernel-core::tty::Termios` to the full POSIX shape; pick safe defaults | None | Planned |
| B | `TCGETS`/`TCSETS`/`TCSETSW`/`TCSETSF` ioctl branches; userspace `tcgetattr`/`tcsetattr` | A | Planned |
| C | `c_iflag` plumbing (`IGNCR` / `INLCR` / `ICRNL` / `IUTF8` / `IXON` / `IXOFF`) | B | Planned |
| D | `c_oflag` plumbing (`OPOST` / `ONLCR`) on both TTY and PTY write paths | B | Planned |
| E | `c_lflag` plumbing (`ICANON` / `ECHO` / `ECHOE`/`ECHOK`/`ECHONL` / `ISIG` / `IEXTEN`) | B | Planned |
| F | `c_cc` VMIN / VTIME timer state in the ldisc | E | Planned |
...
**Acceptance:**
- [ ] `Termios` contains `c_iflag: u32`, `c_oflag: u32`, `c_cflag: u32`, `c_lflag: u32`, `c_cc: [u8; NCCS]` with `NCCS = 19` (matching musl/Linux).
- [ ] A `Termios::cooked_default()` constant returns the baseline ICRNL|IXON|OPOST|ONLCR|ICANON|ECHO|ECHOE|ECHOK|ISIG state with VEOF=0x04, VINTR=0x03, VQUIT=0x1C, VERASE=0x7F, VKILL=0x15, VEOL=0, VSUSP=0x1A, VMIN=1, VTIME=0.
- [ ] A `Termios::raw_default()` constant matches `cfmakeraw` semantics for host tests.
- [ ] Host tests cover: round-trip of every flag through `Termios::cooked_default`; sanity that `raw_default` clears `ICANON|ECHO|ISIG|IEXTEN|OPOST` and the four `c_iflag` mapping bits.
...
**Acceptance:**
- [ ] Every constant uses the Linux numeric value (so musl shims work unchanged).
- [ ] A host test asserts no two flag bits collide within their respective mode word.
...
**Acceptance:**
- [ ] `TCGETS = 0x5401` copies the current `Termios` to user memory.
- [ ] `TCSETS = 0x5402` (`TCSANOW`) applies immediately.
- [ ] `TCSETSW = 0x5403` (`TCSADRAIN`) drains the output queue before applying.
- [ ] `TCSETSF = 0x5404` (`TCSAFLUSH`) drains output and flushes input before applying.
- [ ] Each branch validates the user pointer through the existing `read_user`/`write_user` helpers and returns `-EFAULT` on bad memory.
...
**Acceptance:**
- [ ] The same four ioctl codes are wired against the PTY slave fd.
- [ ] Per-PTY-pair termios state lives in the existing `kernel-core::pty::PtyPair` struct (extended in Track F).
- [ ] PTY master is unaffected — `TCGETS` against the master returns `-ENOTTY`.
...
**Acceptance:**
- [ ] `IGNCR` set → drop incoming `\r`.
- [ ] `INLCR` set → map incoming `\n` to `\r`.
- [ ] `ICRNL` set → map incoming `\r` to `\n`.
- [ ] `IXON` set → incoming `VSTOP` (XOFF, 0x13) suspends output; `VSTART` (XON, 0x11) resumes.
- [ ] `IXOFF` set → ldisc emits XOFF when the input buffer is ≥ 80% full.
- [ ] `IUTF8` set → no behaviour change yet (full effect in Phase 69b); flag round-trips through get/set.
- [ ] Host tests cover each mapping in isolation and combinations (e.g. `IGNCR | ICRNL`).
...
**Acceptance:**
- [ ] `OPOST` cleared → write bytes are delivered verbatim.
- [ ] `OPOST` set + `ONLCR` set → outgoing `\n` is expanded to `\r\n`.
- [ ] `OPOST` set + `ONLCR` cleared → outgoing `\n` is delivered as `\n` only.
- [ ] Host tests cover all four flag combinations on a canned byte stream.
...
**Acceptance:**
- [ ] Same four-combination behaviour as D.1, exercised against the PTY pair.
...
**Acceptance:**
- [ ] `ICANON` cleared → every byte delivered to the ldisc is immediately available for `read`.
- [ ] `ICANON` set → existing canonical line-editor behaviour preserved (verified by the existing Phase 22 tests still passing).
- [ ] A flag-flip mid-stream is observable on the next byte (no buffering of stale state).
...
**Acceptance:**
- [ ] `ECHO` cleared → no characters are echoed on input.
- [ ] `ECHO` set + `ECHOE` set → VERASE prints `^H \b` (the standard erase trio).
- [ ] `ECHO` set + `ECHOK` set → VKILL prints a `\n` after killing the line.
- [ ] `ECHO` set + `ECHONL` set + `ICANON` set → `\n` is echoed even if `ECHO` is cleared.
...
**Acceptance:**
- [ ] `ISIG` cleared → `VINTR` / `VQUIT` / `VSUSP` are delivered as ordinary bytes.
- [ ] `ISIG` set → those control chars trigger Track G's signal path.
- [ ] `IEXTEN` set → `VLNEXT` (literal-next) and `VDISCARD` are honoured; cleared → delivered as ordinary bytes.
...
**Acceptance:**
- [ ] `VMIN > 0, VTIME == 0` → `poll_read_ready` returns false until ≥ `VMIN` bytes are buffered.
- [ ] `VMIN == 0, VTIME > 0` → `poll_read_ready` returns true after `VTIME * 100 ms` even with zero bytes; if a byte arrives sooner, returns true immediately.
- [ ] `VMIN > 0, VTIME > 0` → inter-byte timer; first byte arms the timer; returns true on either `VMIN` reached or `VTIME * 100 ms` since the first byte.
- [ ] `VMIN == 0, VTIME == 0` → poll: always returns true; `read` returns 0 if no data.
- [ ] Host tests cover all four cases with a fake clock.
...
**Acceptance:**
- [ ] `tty_read` consults `Ldisc::poll_read_ready` before blocking; if the predicate is true, returns immediately.
- [ ] When blocked, the read wakes on either a byte-available event or a VTIME deadline (whichever comes first), via the existing `WaitQueue` block/wake primitive.
- [ ] No busy-wait — the timer uses the kernel monotonic clock + a wake-up.
...
**Acceptance:**
- [ ] When `ISIG` set: incoming `VINTR` returns `Action::SignalForeground(Signal::Int)`; `VQUIT` → `Quit`; `VSUSP` → `Tstp`.
- [ ] When `ISIG` cleared: those bytes return `Action::DeliverToReader`.
- [ ] Host tests cover both modes and verify byte delivery is suppressed only when the signal action fires.
...
**Acceptance:**
- [ ] `Action::SignalForeground(Signal::Int)` calls `send_signal_to_group(fg_pgrp, SIGINT)`.
- [ ] Same for `Signal::Quit` → `SIGQUIT` and `Signal::Tstp` → `SIGTSTP`.
- [ ] `tcsmoke isig` (Track I) verifies a self-installed SIGINT handler runs after a `VINTR` byte enters the ldisc.
...
**Acceptance:**
- [ ] `pub struct Termios` matches the kernel ABI byte-for-byte (use `#[repr(C)]`).
- [ ] `tcgetattr(fd) -> Result<Termios, i32>` returns the kernel state or an errno.
- [ ] `tcsetattr(fd, when: TcsetWhen, &Termios) -> Result<(), i32>`.
- [ ] `cfmakeraw(&mut Termios)` clears `IGNBRK|BRKINT|PARMRK|ISTRIP|INLCR|IGNCR|ICRNL|IXON`, `OPOST`, `ECHO|ECHONL|ICANON|ISIG|IEXTEN`, sets VMIN=1, VTIME=0.
- [ ] Re-exports the flag constants from `kernel-core::tty` (or duplicates them with a host-side test asserting equality).
...
**Acceptance:**
- [ ] Subcommands: `round-trip`, `icanon-off`, `echo-off`, `vmin-vtime`, `isig`, `opost-off`. Each prints `TC_SMOKE:<name>:ok` or `TC_SMOKE:<name>:fail <reason>`.
- [ ] `round-trip` modifies every flag bit, calls `tcsetattr` + `tcgetattr`, and asserts bit-perfect equality.
- [ ] `vmin-vtime` covers all four VMIN/VTIME quadrants.
- [ ] `isig` self-installs a SIGINT handler and verifies it runs after a VINTR byte.
...
**Acceptance:**
- [ ] `cargo xtask termios-smoke` boots, runs each `tcsmoke` subcommand, asserts all `:ok`, completes in < 60 s.
- [ ] Wired into the pre-push hook behind `M3OS_TERMIOS_REGRESSION=1`.
...
**Acceptance:**
- [ ] Phase 22 doc notes that the full termios contract landed in Phase 69a.
- [ ] Phase 29 doc cross-refs 69a for the PTY-side ioctl coverage.
...
**Acceptance:**
- [ ] `kernel/Cargo.toml` `version = "0.69.1"`.

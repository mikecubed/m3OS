...
|---|---|---|---|
| A | xHCI host controller driver (ring 3): register discovery, BIOS handoff + reset + run, DCBAA/scratchpad/contexts, command + event rings (ERST, cycle bit, doorbell), MSI-X interrupter + event loop, PORTSC reset/speed | Phase 55b ✅, Phase 67 ✅ | Planned |
| B | Kernel/PCI enablement + driver hosting: Bus Master Enable + MSI-X table programming; stage the `xhci` driver under `/drivers/` and start it | Phase 55b ✅ | Planned |
| C | Bring-up smoke gate + `0.78.0` version bump | A, B | Planned |
...
- [ ] Reads `HCSPARAMS1.MaxPorts` and prints `[xhci] N ports detected`
...
- [ ] Context size (32 vs 64) is selected from `CSZ` and threaded into all later context allocation (A.4)
...
- [ ] **Bus mastering is a hard precondition:** xHCI DMAs nothing and posts no events until PCI Bus Master Enable is set (B.1). The run sequence asserts BME is enabled before `R/S=1`
...
- [ ] Port Status Change events are routed to a handler stub (the enumeration consumer is 78b)
...
- [ ] Port-speed field decoded → Low/Full/High/SuperSpeed → EP0 Max Packet Size = **8 / 8 / 64 / 512** (full-speed corrected via the 78b BSR pre-read + Evaluate Context); host tests assert each speed, **including the SuperSpeed = 512 (`bMaxPacketSize0 = 9`) case**
...
- [ ] `sys_device_irq_subscribe` programs the xHCI controller's PCI MSI-X capability + table (vector, message address + data) with the MSI-X **Enable** bit set (not just kernel-vector allocation); the MSI-X-if-advertised ordering is the `allocate_device_vector` doc comment at `kernel/src/syscall/device_host.rs:1653`
...
**Acceptance:**
- [ ] `xhci` added as a Cargo `member` and to the `bins` array with `needs_alloc = true` (it depends on `kernel-core`)
- [ ] `xhci` binary embedded in `DRIVERS_ENTRIES` (`ramdisk.rs`) at `/drivers/xhci`
- [ ] `xhci.conf` added to `kernel/initrd/etc/services.d/` **and** `init` `KNOWN_CONFIGS`; uses `command=/drivers/xhci`, `type=daemon`, `restart=on-failure`
...
**Acceptance:**
- [ ] QEMU launched with `-device qemu-xhci` (new in the xtask QEMU arg builder)
- [ ] The gate asserts a real `Enable Slot` Command Completion event was consumed off the event ring **via MSI-X** (the driver emits `XHCI_BRINGUP:enable-slot:OK` only on a real interrupt-delivered completion, not on a poll fallback)
- [ ] A serial `[xhci] N ports detected` sentinel alone is explicitly **not** sufficient for PASS
- [ ] Wired as `cargo xtask xhci-bringup-smoke` with an opt-in pre-push gate `M3OS_USB_REGRESSION=1` (mirrors the heavyweight `htop-render-probe` / `compositor-stress` gates and the AGENTS.md hooks table)
...
- [ ] `AGENTS.md` kernel version updated to `v0.78.0` (version string only — the capability-inventory bullet lands at 78c per the keep-it-small policy)
- [ ] `docs/roadmap/README.md` Phase 78a row Status updated to "Complete"; design-doc + task-doc Status headers set to Complete
- [ ] `cargo xtask check` passes
- [ ] Git tag `v0.78.0` — recommended at sub-phase merge (left to the merge step)

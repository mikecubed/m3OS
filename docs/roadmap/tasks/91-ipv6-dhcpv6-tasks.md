...
**Status:** 🟢 Complete — always-on `ipv6-smoke` + `dns6-smoke` gates PASS (incl. full dual-stack TCP over IPv6, `recvmsg`/`sockaddr_in6`, RFC 6724); only the SLAAC/DHCPv6 *live-router* arms are opt-in (no SLIRP RA — see Validation Status)
**Source Ref:** phase-91
...
> **Validation Status (as landed).** The always-on `cargo xtask ipv6-smoke` gate PASSES and validates **live on real frames**: link-local formation (`IPV6_ADDR_OK`), bidirectional **NDP** (the guest answers SLIRP's Neighbor Solicitation with an NA — `NDP_RESOLVE_OK`), AF_INET6 socket creation, `bind6`, the `ping6 ::1` ICMPv6 loopback round-trip through the real `handle_icmpv6` request→reply path (`IPV6_LOOPBACK_OK`/`ICMPV6_ECHO_OK`), and a **full dual-stack TCP-over-IPv6** connection — a listening `AF_INET6` socket + `connect6(::1)` complete the three-way handshake through the kernel's self-address internal loopback and a payload round-trips client→server (`IPV6_SMOKE:tcp:ok`), exercising the family-aware `TcpConnection`, the IPv6 pseudo-header checksum, and `handle_tcp_v6` end-to-end. All `kernel-core` `ipv6`/`icmpv6`/`ndp`/`dhcpv6`/`udp`-v6/`tcp`-v6 parse/build is host-tested (50 tests: 46 across the four new modules + the `udp`/`tcp` `build_v6` checksum tests). **SLIRP limitation found:** QEMU 8.2.2's libslirp does NDP NS/NA but sends **no Router Advertisements** and runs **no DHCPv6 server** (packet-capture-confirmed); the guest's RS, DHCPv6 Information-Request, and DAD NS all go out correctly-formatted but get no reply. So **SLAAC global-address formation, the RA-driven default route, and the stateless/stateful DHCPv6 DNS lease are implemented + host-tested but live-validated only behind the opt-in `M3OS_IPV6_LIVE` arm**, which now attaches the guest to a real LAN via `M3OS_IPV6_TAP=<tap-ifname>` (a TAP bridged to a segment with a real IPv6 router) rather than SLIRP — mirroring the established `*_NET` opt-in pattern. **SLAAC was demonstrated end-to-end against a real home router** this way: the guest formed a real `/64` global address from the router's RA and the full `ipv6-smoke` gate PASSed. Two robustness items surfaced from that real-router run and landed: an **RFC 4861 Router-Solicitation retransmit** (up to 3, ~4 s apart, until a global is configured — a single missed RA no longer strands the host) and concise **RA-reception diagnostics** (`[ndp] RA rx …` / parse-fail / checksum-drop). Per-run acquisition stays best-effort (bounded by the router's RA cadence and the deferred MLD — without MLD the guest never formally joins `ff02::1`, so multicast RA delivery through a bridge is not guaranteed), which is why this stays opt-in rather than an always-on gate. The `CURL6_OK` real-internet TCP arm is likewise opt-in (needs a routable global v6 address, which requires the real router). **Track D** landed too: the always-on `dns6-smoke` (in the main `smoke-test` via the smoke-runner) asserts dual-stack `getaddrinfo("localhost", AF_UNSPEC)` returns both families ordered per RFC 6724 from the staged `/etc/hosts` (`RFC6724_OK`, CI-deterministic), with a soft real-internet AAAA probe (`M3OS_IPV6_LIVE`-class). `sys_recvmsg_inet6` is implemented too — `sys_recvmsg_inet` is now family-aware and fills a 28-byte `sockaddr_in6` for AF_INET6, validated by the `ipv6-smoke` `recvmsg` case. **No follow-ups remain in this phase** beyond the explicitly out-of-scope items (privacy extensions, DAD, MLD, IPsec, mobility, DHCPv6-PD, Happy Eyeballs) and the opt-in live-router SLAAC/DHCPv6/`CURL6` arms.
...
**Acceptance:**
- [x] A UDP datagram over IPv6 checksums correctly with the IPv6 pseudo-header — host-tested in `kernel-core` (`udp::build_v6` folds to 0 on verify, a corrupted byte does not, and the RFC 768 `0→0xffff` edge is asserted); the kernel `udp::build_v6` delegates to this pure logic. The live `DHCPV6_DNS_OK` exercise of the path rides the opt-in `M3OS_IPV6_LIVE` arm (SLIRP runs no DHCPv6 server).
...
- [x] `sys_recvmsg_inet6` fills a correct `sockaddr_in6` in `msg_name` (the family-aware `sys_recvmsg_inet` writes a 28-byte `sockaddr_in6` for AF_INET6) — proven by the `ipv6-smoke` `recvmsg` case (a v6 UDP datagram looped via `::1`, read with `recvmsg`, asserting `sin6_family==AF_INET6` + `msg_namelen==28`); **IPv4 sockets are unaffected** (`dns-smoke` still PASSes).
...
- [x] The RA source is installed as the IPv6 default gateway in `config`; a zero router-lifetime RA does not install a default route (and from the *current* router withdraws it, RFC 4861 §6.3.4) — the RA→config decision is host-tested in `kernel-core` (`ndp::ra_to_config`: gateway Install/Clear/Leave + zero-lifetime), and `handle_router_advertisement` applies it. The live install rides the opt-in `M3OS_IPV6_LIVE` arm (no SLIRP RA).
- [x] The M flag is parsed and surfaced on the RA (`RouterAdvertisement::managed`, host-tested) and branched on in `ra_to_config`/`handle_router_advertisement` (M=1 → DHCPv6/route-only, M=0 → SLAAC), so C.2 knows whether to start the stateful exchange. The live M=1→Solicit auto-drive rides the opt-in arm.
...
- [x] On a network advertising a prefix, the host forms a correct global SLAAC address and installs the default route (`SLAAC_ADDR_OK`) — **demonstrated live against a real home router** via a TAP bridge (`M3OS_IPV6_TAP=<ifname>` + `M3OS_IPV6_LIVE=1`): the guest formed a real `2001:db8`-class `/64` global address from the router's RA and the full `ipv6-smoke` gate PASSed. (The premise "SLIRP sends RAs" was wrong — QEMU libslirp does not — so this is opt-in, not always-on; per-run acquisition is best-effort, bounded by the router's RA cadence and m3OS's deferred MLD, so the guest does not always catch an RA inside the gate window.)
...
**Acceptance:**
- [x] The Solicit→Advertise→Request→Reply state machine parses the IA_NA address and the DNS-server option and emits the Request echoing the SERVERID + offered address — host-tested in `kernel-core` (`dhcpv6` `stateful_full_walk`, `parse_advertise_round_trip`, `parse_reply_round_trip`; 20 tests).
- [ ] The four-message exchange completes **live** against a real DHCPv6 server (E.2 stateful arm, `M3OS_IPV6_LIVE`) — SLIRP runs no DHCPv6 server.
...
**Acceptance:**
- [ ] `getaddrinfo("github.com", …)` returns **both** `AF_INET` and `AF_INET6` `addrinfo` entries (the AAAA arm proven by `AAAA_RESOLVE_OK`; opt-in real-internet via `M3OS_IPV6_LIVE=1`, skip-with-reason otherwise — mirroring `dns-smoke`).
...
- [ ] With **no** usable global IPv6 source address configured, a real dual-stack name's IPv4 result is selected first (no v6-preference black-hole) — exercised on the opt-in `M3OS_IPV6_LIVE` arm alongside `AAAA_RESOLVE_OK`.
...
- [x] The stateful-DHCPv6 + real-egress arms are gated behind `M3OS_IPV6_LIVE=1` and **skip-with-reason** when unset (no CI dependency on a host IPv6 daemon or real internet).
...
**Symbol:** `M3OS_IPV6_REGRESSION=1` → runs `ipv6-smoke` (PASS, not SKIP); the AGENTS.md regression-table row describing the gate, its CI-deterministic vs opt-in arms, and `M3OS_IPV6_LIVE=1`
...
- [x] AGENTS.md gains the `ipv6-smoke` / `M3OS_IPV6_REGRESSION=1` regression-table row (CI-deterministic arms always-on; network arms behind `M3OS_IPV6_LIVE=1`).
...
- **Honesty / explicit non-goals:** `ping6 ::1` is an ICMPv6 loopback short-circuit, not a routed `lo` interface (m3OS has none). Privacy extensions (RFC 4941), DAD, MLD/MLDv2, IPsec, mobility/segment routing, DHCPv6-PD, and Happy Eyeballs (RFC 8305) are deferred. RFC 6724 selection relies on musl's existing sort — the phase supplies correct source-address inputs, it does not re-implement the sort. Stateful DHCPv6 leasing + real-internet AAAA/curl are opt-in (`M3OS_IPV6_LIVE=1`), since QEMU SLIRP `ipv6=on` does NDP NS/NA but sends **no** Router Advertisements and runs **no** DHCPv6 server (packet-capture-confirmed) — SLAAC, the RA-driven default route, and the DHCPv6 DNS lease all need a real router (a TAP bridge), not SLIRP.

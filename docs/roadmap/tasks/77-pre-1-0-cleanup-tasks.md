...
- [ ] `htop` launched from `term` shows a non-empty process list — **NOT MET (regression still reproduces).** A headless QMP screendump (`cargo xtask htop-render-probe`) shows htop fully rendered but `Tasks: 0, 0 thr, 0 kthr` with an empty process table. Kernel `open()` tracing: per PID htop opens `/proc/<pid>` (dir) + `/proc/<pid>/statm` but **never `/proc/<pid>/stat`**, so it discards every process. `ps -e` works because it reads `/proc/<pid>/status`, which masked the regression and led to this item being wrongly marked done. Root cause of the skipped `stat` read is not yet pinned — re-opened in `docs/handoffs/2026-05-20-htop-zero-processes.md` (`phase_77_finding`). The status-field + procfs-enumeration work above is correct and still useful; the htop blocker is the unread `/proc/<pid>/stat`.

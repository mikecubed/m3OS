...
**Acceptance:**
- [x] The 2026-04-25 spin is root-caused — **but NOT as the multithread-join lost-wake.** That attribution (commits `6f57fbc`/`b6f517b`/`e8f5b74`) was investigated and **retired** by the 2026-05-29 deep trace (`docs/handoffs/2026-05-29-ssh-disconnect-lost-wakeup.md`): the SSH-disconnect freeze is a **userspace `async-rt` missed-edge stall**, not a kernel lost futex wake. A transient PTY-master `POLLHUP` that fails to wake the sshd relay parked in `WaitWake` stalls the purely edge-driven executor. Fixed by the idle-liveness backstop `Executor::requeue_all` in `userspace/async-rt/src/executor.rs` (commit `7ce392a`), plus the `sunset-local` clean-logout (`server_session_exit`, commit `4eb0ce2`). The futex dual-key wake (`b6f517b`) and the scheduler `on_cpu` cross-core defer (`6f57fbc`) are real fixes for the **Track C pthread/TLS join hang** and a **distinct cross-core deadlock class** respectively — neither is what unwedged the disconnect.
- [x] After a client `exit`, the sshd session task terminates — the EOF-driven `cleanup` (close PTY master first → shell EOF-exits → bounded `nanosleep` reap, SIGKILL last resort) shipped in `6f57fbc`; the async-rt backstop (`7ce392a`) removes the residual relay stall and its final flush is bounded so `close(sock_fd)` always runs
...
- [x] `getaddrinfo("github.com", ...)` resolves end to end via the `userspace/dns-smoke` binary — **fixed and verified 2026-05-28 by a one-time manual serial-trace observation:** the query egressed with a valid ephemeral source port, the reply (44 bytes from `10.0.2.3:53`) was delivered to the resolver, which accepted it (source-address validation passes) and `getaddrinfo` returned the A record (`dns-smoke` printed `DNS_SMOKE:PASS <ip>` — only emitted on a real resolution); `recvmsg` delivered `n=44` and the resolver's `poll` busy-spin collapsed from **11,918 iterations → 4**. **Caveat:** the standing `cargo xtask smoke-test` gate is *soft* — the smoke-runner emits `SMOKE:dns-smoke:PASS` for both a real resolution AND a `DNS_SMOKE:SKIP` (no outbound DNS in the sandbox), so that marker proves only that the resolver path runs without hanging, **not** that a resolution succeeded. To regression-protect a real resolution, run with outbound DNS reachable and set `M3OS_DNS_REGRESSION=1` (pre-push fails on a SKIP).
...
- [x] **Follow-up (multi-segment retransmit queue):** the original D.2 implementation tracked only ONE outstanding segment (a single `Option<RetransmitSeg>` slot) — any second segment sent before the first was ACKed was unprotected, and a FIN sent with data still in flight was dropped silently on loss. Replaced with a host-tested `kernel_core::net::tcp::RetransmitQueue` (a `VecDeque` of unacked segments + the single RFC 6298 timer that times the oldest). `on_ack` prunes every cumulatively-acked segment from the front and restarts the timer (§5.2/§5.3); `service_rto` retransmits the earliest unacked segment (§5.4). Send is now flow-controlled: `tcp_send` caps outstanding bytes at `min(peer_window, MAX_INFLIGHT_BYTES=64 KiB)` and returns the accepted byte count; `sys_sendto`'s TCP arm blocks (yields), returns `EAGAIN` (non-blocking), or `EPIPE` (connection reset/closed) when the window is full — mirroring the recv loop. **11 host tests** in `kernel-core` pin the queue (cumulative/partial ACK, oldest-first retransmit, SYN/FIN seq-len, MAX_RETRANSMITS reset, Karn suppression, u32-wrap ACK coverage).
- [x] The connection-state machine is otherwise unchanged — the retransmit hooks are additive (`arm_retransmit`/queue `push` on SYN/data/FIN send, `on_ack` in the existing ACK arms); `cargo xtask check` (clippy + fmt + host tests), `smoke-test`, `ssh-e1000-banner-check`, and the regression suite all pass.

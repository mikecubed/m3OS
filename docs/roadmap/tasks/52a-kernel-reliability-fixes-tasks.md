...
|---|---|---|---|
| A | IPC blocking return state fix | None | Planned |
| B | Sunset write waker fix | None | Planned |
| C | Thread exit clear_child_tid | None | Planned |
| D | Exec signal action reset | None | Planned |
...
**Acceptance:**
- [ ] `dispatch()` captures `saved_user_rsp = per_core_syscall_user_rsp()` and `pid = current_pid()` at entry
- [ ] `dispatch()` calls `restore_caller_context(pid, saved_user_rsp)` before every return path
- [ ] On SMP4, a userspace task calling `ipc_recv` in a loop does not crash or observe a corrupted stack after 10,000 iterations
- [ ] The `copy_to_user` reliability bug investigation doc is updated with the fix status
...
**Acceptance:**
- [ ] FUTEX_WAIT path captures `saved_user_rsp` before blocking and calls `restore_caller_context` after wakeup
...
**Acceptance:**
- [ ] `Channel::wake_write()` uses `self.write_waker.take()` for the `ChanData::Normal` arm
- [ ] SSH session displays shell prompt within 2 seconds of authentication without any client keystroke
- [ ] The SSHD hang analysis doc (`docs/appendix/sshd-hang-analysis.md`) is updated with the fix status
...
**Acceptance:**
- [ ] When `write_channel` returns `Ok(0)` and `pty_pending_len > 0`, the relay task calls `set_channel_write_waker` before sleeping
...
**Acceptance:**
- [ ] When a thread with `clear_child_tid != 0` exits, the kernel writes `0u32` to that address via `copy_to_user`
- [ ] The kernel calls `futex_wake(clear_child_tid, 1)` after the write
...
**Acceptance:**
- [ ] After ELF loading and before `enter_userspace`, `sys_execve` iterates `signal_actions` and resets any `Handler` disposition to `Default`
- [ ] `Ignore` and `Default` dispositions are preserved (not reset)

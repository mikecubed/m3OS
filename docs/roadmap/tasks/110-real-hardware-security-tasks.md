...
**Still open in A.3b:** page-isolating GDT/IDT/TSS into a `cpu_entry_area` to close the A.3a adjacent-`.data` leak *(A.3b part 4)*; and wiring `build_user_half` onto `AddressSpace` so a per-process user PML4 exists to load *(A.3b part 5, consumed by A.4)*.

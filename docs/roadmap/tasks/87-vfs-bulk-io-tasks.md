...
> **Post-review hardening (PR #238 final audit).** A final correctness/completeness audit of the landed write-back path produced three in-reach robustness fixes (no behaviour change on the happy path, validated by the same gates):
> - **Pure in-place overwrite now gets a periodic device FLUSH.** The periodic write-back flush gated its device `block_flush` on `has_pending_writes()` (deferred pointer blocks OR `meta_dirty_ops>0`). A pure overwrite of an already-allocated file allocates nothing and defers no pointer block, so it buffered nothing in vfs_server yet left dirty bytes in the device/host write-back cache — and so was **never** periodically flushed, silently breaking the documented "host crash → bounded ~5 s" guarantee for overwrite-heavy clients. Added a `writes_since_flush` flag (set by every device write, cleared after `block_flush`); the periodic FLUSH now fires on it, making the guarantee hold for **all** write patterns. Idle-path only — zero throughput impact on the active install path.
> - **`free_block` purges the freed block from the write-back buffer + read cache.** `dirty_blocks` is normally drained at every request boundary, so the freed-then-reused aliasing was unreachable — but the boundary flush is best-effort (its result is discarded on I/O error), leaving the invariant implicit and fragile. `free_block` now explicitly drops the block from `dirty_blocks`/`block_cache`, so a reallocated block can never serve stale deferred pointer bytes on read nor have `flush_dirty_blocks` clobber its new data.
> - **`claim_block_run` decrements free counts saturatingly.** A stale-counter remount after an unclean crash (on-disk free summary lower than the bitmap's actual free bits) could make a multi-block run claim more bits than the counter admitted, wrapping the `u16`/`u32` free count to a bogus huge value. `saturating_sub` degrades to the documented fsck-reconcilable state (group looks full, surplus bits leak until fsck) instead.
>
> And one CI-robustness fix: `ahci-rw-smoke` (the only CI coverage of the ring-3 write path) gained a `M3OS_AHCI_RW_REQUIRE_PASS=1` mode — set on its always-on `pr.yml` step — that turns a musl-absent SKIP into a hard failure, so dropping `musl-tools` from CI can't make the proof vacuously green (mirrors `tls-smoke`/`dns-smoke` PASS-not-SKIP).
...
| F | (Optional) bulk page-grant transfer + `.m3pkg` compression | B | Deferred (optional / additive — nothing depends on F; the milestone goal + all acceptance criteria are met without it) |
...
- [x] Byte-for-byte identical output vs the per-block path, verified by a `kernel-core` host test that crosses the single→double-indirect boundary. **As-built:** `coalesced_read_is_byte_identical_to_per_block` (+ hole / run-jump / cap-split / EOF cases) drives an arithmetic `mock_resolve`, and `coalesced_read_byte_identical_across_real_indirect_blocks` drives a **real** single- + double-indirect pointer-block walk (reading pointer blocks from an in-memory store, exactly as `resolve_block` does) — proving the coalescer **merges** a physically-contiguous run across the direct→single-indirect (logical 11→12) and single→double-indirect (267→268) transitions and **breaks** only at a real discontiguity (jump) or hole, byte-identical to a naive per-block reader.
...
> **Deferred — legitimate (not an unfinished must-have).** All three *measurable* design-doc acceptance criteria (≥4× wall-clock — see the op-count reframing in Status, ≤512 block requests, no multi-second UI freeze) are already met by Tracks B + C.2 + D **without** readahead. More importantly, meaningful readahead is **not achievable in the current architecture**: `vfs_server` is a single-threaded synchronous rendezvous (`ipc_recv_msg` → `handle_request` → `ipc_reply`), and `read_sectors` calls the blocking `block_read` syscall — there is no worker thread or async submission queue. A *synchronous* prefetch in a single-threaded server hides no latency (there is no concurrent consumer to overlap with); the only readahead that would help is overlapped async/worker-thread submission, which depends on the separately-deferred async SQ/CQ block ring (see Design doc → Deferred Until Later). So C.1 is correctly deferred behind that driver-side work, not left undone within this phase's reach.
...
> **As-built:** "write-back" landed as three vfs_server changes rather than a `close`/`fsync` buffer: (1) **deferred metadata flush** — the sb/BGD free-count summaries flush at most once per `META_FLUSH_THRESHOLD` (256) alloc/free ops instead of per allocation (the bitmaps stay authoritative + persisted, so a crash is `fsck`-reconcilable — exactly like real ext2's lazy block-group counts), **and the residual is drained on SIGTERM** by `flush_metadata_if_dirty` (a `rt_sigaction_simple` handler sets a flag; the server loop flushes + exits before init's SIGKILL grace) so an orderly shutdown persists the summaries too — the default SIGTERM action would otherwise terminate the server with the residual unflushed; (2) **data-write coalescing** — `write_file_data` accumulates physically-contiguous whole blocks and flushes them in one `write_block_run` (multi-block `block_write`); (3) **zero-fill skip** — `allocate_data_block(zero_fill=false)` for full-block writes (the run payload overwrites the whole block, so the separate zero-write is redundant; partial head/tail blocks keep `zero_fill=true`, so a newly-allocated partial block never leaks stale content — covered by the new `pkg verify` read-back-compare in `vfs-bulkio-smoke`).
...
- [~] A QMP-injected keystroke is echoed by `term` within 500 ms during a bulk install. **As-built — the *intent* (no multi-second freeze) is met and DETERMINISTICALLY ENFORCED; the literal keystroke-echo probe is a noted follow-up.** Two facts make the literal probe the wrong instrument here: (1) **keystroke echo is architecturally independent of vfs_server** — `term`'s per-keystroke render loop makes zero VFS calls (its only VFS read is a one-shot font load at startup), so the echo path is PS/2/USB → kbd_server → display_server → term → framebuffer, exercising the scheduler/compositor, *not* the bulk-I/O path this phase fixed. (2) The actual fairness mechanism — fewer device writes per WRITE request ⇒ shorter per-request latency — is **already guarded deterministically** by `vfs-bulkio-smoke`'s `write_calls ≤ 2,400` assertion: a regression that reintroduced the per-block bitmap writes (the root cause of the >1 s WRITE requests) would blow that bound and fail the gate. The slow-req serial proxy confirms the worst vfs *request* dropped multi-second → < 1 s (>1 s WRITEs eliminated). A wall-clock GUI keystroke gate would instead be **host-speed-flaky** (it measures scheduler latency, not op counts) — so it is deliberately left as a follow-up (reusing the `usb-smoke` press_key→screendump→diff plumbing), together with a stricter <500 ms request bound (a smaller write cap, throughput-traded).
...
- The motivating regression is `pkg install python` (Phase 85c); the heavy-I/O consumers are Phases 89 (Node.js) and 90 (Claude Code).

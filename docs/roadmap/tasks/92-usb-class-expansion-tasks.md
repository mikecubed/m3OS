...
- ⏳ **D.5 + H.4** (zero-copy page-grant `SubmitTransfer`) — **deferred**: the functional goal (large/4096-block transfers) is delivered by the bulk-OUT fix above. The *literal* page-grant (a single >`USB_MSG_MAX` zero-copy transfer) is **blocked on missing kernel IOMMU infrastructure** — a ring-3 driver cannot IOMMU-map an externally-granted page; it needs a new capability-gated kernel syscall (or an inverted grant). Marginal throughput win over chunked inline; scheduled as a focused follow-up.
...
> **Status (revised after investigation): the functional goal is met via the inline path; the literal page-grant is blocked on missing kernel IOMMU infrastructure.**
>
> The multi-sector inline path was the real blocker, and it is now **fixed + validated**: a real-world **4096-byte-block** ext2 USB stick mounts + reads + writes over `SubmitBulkIn`/`SubmitBulkOut` (each 8-sector block I/O = a 7-sector + 1-sector inline pair). The fix root-caused a **bulk-OUT recv-truncation bug** (the xHCI server `recv`d with the 1522-byte Ethernet-MTU buffer → truncated >1522-byte `SubmitBulkOut` → device timeout) + a SHORT_PACKET completion-filter inconsistency. So D.5's functional outcome (large/4096-block transfers) is delivered **without** the page-grant.
>
> The **literal H.4 page-grant `SubmitTransfer`** (zero-copy device DMA into a daemon-granted page, for a *single* >`USB_MSG_MAX` transfer = fewer IPC round-trips on multi-MiB I/O) is **blocked on kernel infrastructure that does not exist**: a ring-3 driver can only IOMMU-map buffers it allocates itself via `sys_device_dma_alloc` (its own device cap); there is **no syscall to IOMMU-map an externally-granted page into a device's IOMMU domain**. Implementing it requires either (a) a new capability-gated kernel IOMMU-map-grant syscall, or (b) inverting the grant (the xHCI server allocates the DmaBuffer and shares it with the daemon) — both are substantial, and the throughput win over the now-working chunked inline path is marginal. **Deferred** to a focused follow-up. **Note:** H.6 fixed the inline budget at 7 sectors (3584 B) — the reply carries 3 bytes of wire overhead.
...
**Acceptance:**
- [~] Transfers ≤ 4096 bytes use inline `SubmitBulkOut`/`PollBulkIn`; transfers above it use the H.4 page-grant path. — the **inline path now works for all block I/O** (the multi-sector bulk-OUT recv-truncation bug is fixed; a 4096-block ext2 mounts via 7+1-sector inline chunks). The page-grant overflow path for a *single* >`USB_MSG_MAX` transfer is **deferred** — it needs kernel IOMMU-map-grant infrastructure (see H.4).
- [~] A large sequential read (e.g. a multi-MiB file copy off `/mnt/usb0`) completes and verifies byte-identical. — multi-block reads complete + verify byte-identical via chunked inline transfers (`usb-mount-smoke` reads files); the page-grant fewer-round-trips optimization for very large single transfers is deferred (H.4 kernel-infra dependency).

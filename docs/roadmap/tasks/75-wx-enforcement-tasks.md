...
**Symbol:** N/A
**Why it matters:** Phase 89 Node.js and Phase 90 Claude Code will require JIT code generation; developers need the correct pattern before they hit `EINVAL` from `mprotect`.
...
- [x] A note references Phase 89 (Node.js) as the first consumer
...
**Symbol:** N/A
**Why it matters:** A learner-friendly doc scoped to Phase 75 consolidates the W^X enforcement story — ELF loader, `mprotect` guard, heap/stack NX, and the JIT exception pattern — in one place, so readers do not need to cross-reference Phase 11, Phase 36, and Phase 89 documentation.

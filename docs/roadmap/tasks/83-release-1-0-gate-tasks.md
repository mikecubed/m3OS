...
| D | **Documentation + version alignment**: bump kernel `0.82.0` → `0.83.0`; create + index the learning doc `docs/83-release-1-0-gate.md`; flip the roadmap README row + Tasks link; align top-level `README.md`/`docs/README.md`; reconcile the evaluation R10 doc + stale release cross-refs; design-doc + AGENTS.md closeout | A, B, C | ✅ Complete |
...
- **Authoritative artifact created:** `docs/release/1.0-release-gate.md` (Tracks A+B+C as one coherent, internally-consistent document — the A↔B mutual-exhaustiveness invariant was the single most important review check and was verified zero-orphan in both directions).
- **Independent review:** two reviewers ran on the integrated diff. The release-gate reviewer found one blocker — `audio-smoke`/`bell-smoke` were cited as backing the `Supported` AC'97 row but missing from the Gate Bundle (orphan gates) — fixed by adding them to the Class (2) bundle + Release Checklist; mutual exhaustiveness re-verified. The doc-alignment reviewer returned PASS and flagged stale `Phase 58/59–62` references in the Phase 53 doc, which were also reconciled.
- **Validation run locally (not just CI), all PASS:**
  - `cargo xtask check` — clippy clean, rustfmt correct, all host-test crates pass (kernel `0.83.0` builds clean).
  - `cargo xtask smoke-test --timeout 300` — 22 steps; build line `Compiling kernel v0.83.0`.
  - `cargo xtask regression --timeout 90` — 11 passed, 0 failed.
  - `cargo xtask session-smoke` — 3 steps; step 1 matched the `[m3os] Hello from kernel` banner, directly confirming the boot banner reports `0.83.0`.
- **Grep-verified invariants:** no free-text "works"/"should work" in any matrix cell; matrix uses only the 6 closed status words; all 14 AGENTS.md `M3OS_*_REGRESSION` env vars present verbatim; no `1.0.0` m3OS crate version anywhere; all new/edited relative doc links resolve.
- The remaining opt-in hardware gates (USB/AHCI/HDA/multi-NIC/Wi-Fi screenshot+WAV probes) are the *maintainer release-cut* checklist the contract prescribes — several SKIP without specific hardware or opt-in env vars — not a merge prerequisite for this documentation phase (no-new-runtime-code rule).

...
**File:** `userspace/vfs-throughput-probe/src/main.rs` (new probe binary); `xtask/src/main.rs` (`cmd_vfs_throughput_smoke`, `vfs_thrput_write_ceiling`/`vfs_thrput_read_ceiling`, `vfs_throughput_smoke_steps`); `.githooks/pre-push` (`M3OS_VFS_THROUGHPUT_REGRESSION` arm); `AGENTS.md` (regression-gate table row).
**Symbol:** `cmd_vfs_throughput_smoke`; `vfs_thrput_write_ceiling`/`vfs_thrput_read_ceiling`; `VFS_THRPUT:*` sentinels; `SMOKE_EXIT_VFS_THROUGHPUT_FAILED`
...
**Acceptance:**
- [x] A measurement reports the device block-request delta (`/proc/blkstats`) on an 8 MiB write + read-back. **Measured baseline (fresh disk, TCG): `write_calls_delta` ≈ 649, `read_calls_delta` ≈ 134** — the write count is dominated by ext2 metadata (inode + bitmaps + single/double-indirect blocks), reads touch far less, hence the asymmetry. The gate run produced these and **PASSES**. As implemented: the probe writes 8 MiB in 256 KiB chunks (mkdir-ing its parent dir first for a fresh disk), reads back and byte-verifies, emitting `VFS_THRPUT:write_calls_delta=<d>` / `VFS_THRPUT:read_calls_delta=<d>` / `VFS_THRPUT:verify=ok`.
- [x] A gate (`cmd_vfs_throughput_smoke` / `cargo xtask vfs-throughput-smoke`) asserts `verify=ok` and direction-specific ceilings anchored to the per-block regression it guards against (`n_bytes/4096` ≈ 2048 for 8 MiB): `write_calls_delta ≤ ¾·per-block` (1536) and `read_calls_delta ≤ ½·per-block` (1024) — comfortable headroom over the ~649/~134 baseline so a noisy boot doesn't flake the opt-in gate, yet well under a per-block collapse. Opt-in via `M3OS_VFS_THROUGHPUT_REGRESSION=1` in `.githooks/pre-push`; no special host toolchain required (probe is always embedded in the ramdisk). `#[cfg(test)]` unit tests for both ceilings (8 MiB values, baseline-clearance, per-block-regression-trips-both) run under `cargo xtask check`. **Validated: `cargo xtask vfs-throughput-smoke` PASSES** (write 649 ≤ 1536, read 134 ≤ 1024, verify=ok).

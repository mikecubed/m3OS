...
|---|---|---|---|
| A | Symlink support (tmpfs + ext2 + path resolution) | — | Complete |
| B | Hard link support (ext2) | — | Complete |
| C | Permission-model completion and umask | A | Complete |
| D | `/proc` filesystem | A | Complete |
| E | Device nodes | — | Complete |
| F | `statfs` / `fstatfs` syscalls | — | Complete |
| G | Userspace utilities | A, B | Complete |
...
**Acceptance:**
- [x] `TmpfsNode::Symlink(SymlinkData)` variant added with `target: String` field
- [x] `TmpfsStat` extended with `is_symlink: bool` field
- [x] Existing `Tmpfs::stat()` returns `is_symlink = true` for symlink nodes
...
**Acceptance:**
- [x] `create_symlink(path, target)` creates a `TmpfsNode::Symlink` at `path`
- [x] `read_symlink(path)` returns the target string for symlink nodes
- [x] `read_symlink()` on a non-symlink returns an error
- [x] `unlink()` deletes symlink nodes (existing delete path works)
- [x] Host tests validate round-trip create/read symlinks
...
**Acceptance:**
- [x] `create_symlink(parent_ino, name, target)` allocates an inode with `S_IFLNK` mode
- [x] Short targets (<=60 bytes) stored inline in `Ext2Inode::block` array
- [x] Long targets (>60 bytes) stored in an allocated data block
- [x] Directory entry created with `EXT2_FT_SYMLINK` (7) file type
- [x] `read_symlink(ino)` returns the target string from inline or block storage
- [x] Inode `links_count` set to 1 on creation
...
**Acceptance:**
- [x] Syscall 88 dispatches to `sys_symlink(target_ptr, linkpath_ptr)`
- [x] Syscall 266 dispatches to `sys_symlinkat(target_ptr, dirfd, linkpath_ptr)`
- [x] Routes to `Tmpfs::create_symlink()` for `/tmp/*` paths
- [x] Routes to `Ext2Volume::create_symlink()` for ext2-backed paths at `/` (with `/data/*` as legacy fallback if present)
- [x] Returns `NEG_EROFS` for ramdisk paths
- [x] Returns `NEG_EEXIST` if linkpath already exists
...
**Acceptance:**
- [x] Syscall 89 dispatches to `sys_readlink(path_ptr, buf_ptr, bufsize)`
- [x] Syscall 267 dispatches to `sys_readlinkat(dirfd, path_ptr, buf_ptr, bufsize)`
- [x] Copies symlink target into userspace buffer, returns byte count
- [x] Returns `NEG_EINVAL` if path is not a symlink
- [x] Truncates target to `bufsize` without NUL terminator (POSIX behavior)
...
**Acceptance:**
- [x] Path resolution follows symlinks in each component (not just the final one)
- [x] FS-aware lookup in `syscall.rs` detects tmpfs symlinks and substitutes targets
- [x] `Ext2Volume::resolve_path()` detects `is_symlink()` inodes and follows them
- [x] Absolute symlink targets restart resolution from root
- [x] Relative symlink targets resolve relative to the symlink's directory
- [x] `ELOOP` returned after 40 hops (symlink loop detection)
- [x] `O_NOFOLLOW` flag in `open()` returns `NEG_ELOOP` if final component is a symlink
...
**Acceptance:**
- [x] `sys_linux_fstatat()` honors `AT_SYMLINK_NOFOLLOW` for the final path component
- [x] Syscall 6 (`lstat`) routes through the non-following path instead of behaving like `stat`
- [x] `userspace/syscall-lib` exposes an `lstat`-style wrapper using the existing byte-slice API conventions
- [x] `ls` / `stat` callers can request symlink metadata without following the target
...
**Acceptance:**
- [x] `NEG_ELOOP` = `(-40_i64) as u64` added to error constants block
- [x] `NEG_EXDEV` = `(-18_i64) as u64` added to error constants block
...
**Acceptance:**
- [x] `pub fn symlink(target: &[u8], linkpath: &[u8]) -> isize` wrapper added
- [x] `pub fn readlink(path: &[u8], buf: &mut [u8]) -> isize` wrapper added
- [x] `pub fn symlinkat(target: &[u8], dirfd: i32, linkpath: &[u8]) -> isize` wrapper added
- [x] `pub fn readlinkat(dirfd: i32, path: &[u8], buf: &mut [u8]) -> isize` wrapper added
...
**Acceptance:**
- [x] `create_hard_link(parent_ino, name, target_ino)` creates a dir entry for existing inode
- [x] Inode `links_count` incremented by 1 and written back to disk
- [x] Returns error if target inode is a directory (hard-linking directories is forbidden)
- [x] Returns error if target inode is on a different filesystem
...
**Acceptance:**
- [x] `unlink()` removes the directory entry immediately but open ext2 FDs keep working
- [x] Final block/inode reclamation happens only after `links_count == 0` and the last ext2 FD closes
- [x] `sys_linux_close()` participates in the final cleanup path for unlinked ext2 files
- [x] Hard-linked files with remaining names are unaffected by the extra FD-lifetime tracking
...
**Acceptance:**
- [x] Syscall 86 dispatches to `sys_link(oldpath_ptr, newpath_ptr)`
- [x] Syscall 265 dispatches to `sys_linkat(olddirfd, oldpath_ptr, newdirfd, newpath_ptr, flags)`
- [x] Routes to `Ext2Volume::create_hard_link()` for ext2 paths
- [x] Returns `NEG_EPERM` when attempting to hard-link a directory
- [x] Returns `NEG_EXDEV` when oldpath and newpath are on different filesystems
- [x] Returns `NEG_EROFS` for ramdisk or tmpfs targets (no tmpfs hard links)
...
**Acceptance:**
- [x] `pub fn link(oldpath: &[u8], newpath: &[u8]) -> isize` wrapper added
- [x] `pub fn linkat(olddirfd: i32, oldpath: &[u8], newdirfd: i32, newpath: &[u8], flags: i32) -> isize` wrapper added
...
**Acceptance:**
- [x] `getdents64()` checks directory read permission before returning entries
- [x] Traversal still requires execute/search permission on the directory path
- [x] Returns `NEG_EACCES` for callers lacking the required bits
- [x] Root bypass remains unchanged
...
**Acceptance:**
- [x] `stat()`/`open()` permission checks apply to the resolved target after symlink traversal
- [x] `lstat()` and `readlink()` use the symlink's own metadata for the final component
- [x] Parent-directory search permission is still required even when the final component is a symlink
- [x] Synthetic `/proc` and `/dev` entries continue to expose stable root-owned metadata
...
**Acceptance:**
- [x] `Process` struct has `umask: u16` field, default 0o022
- [x] `fork()` copies parent's `umask` to child
- [x] `exec()` preserves the umask
...
**Acceptance:**
- [x] Syscall 95 dispatches to `sys_umask(mask)`
- [x] Returns the previous umask value
- [x] Sets new umask to `mask & 0o777`
- [x] `open(O_CREAT)` applies umask: `effective_mode = mode & ~umask`
- [x] `mkdir()` applies umask: `effective_mode = mode & ~umask`
...
**Acceptance:**
- [x] `pub fn umask(mask: u32) -> isize` wrapper added
...
**Acceptance:**
- [x] `kernel/src/fs/procfs.rs` module created and registered in `fs/mod.rs`
- [x] `FsTarget::Proc(String)` variant added to `FsTarget` enum
- [x] `resolve_fs_target()` routes `/proc/*` paths to `FsTarget::Proc`
- [x] `FdBackend::Proc` variant added for open procfs file descriptors
...
**Acceptance:**
- [x] Reading `/proc/<pid>/status` returns formatted process metadata
- [x] Output includes: Name, State, Pid, PPid, Uid, Gid fields
- [x] Returns `NEG_ENOENT` for non-existent PIDs
- [x] Content generated on-read from `PROCESS_TABLE`
...
**Acceptance:**
- [x] Reading `/proc/<pid>/cmdline` returns NUL-separated argument strings
- [x] Returns empty for processes with no stored arguments
- [x] Process struct stores `cmdline: String` (set during `execve`)
...
**Acceptance:**
- [x] Reading `/proc/<pid>/maps` emits one line per mapped region
- [x] Each line includes start/end addresses, rwx-style permissions, and a path or region label
- [x] Anonymous mappings, ELF segments, stack, and heap regions are represented consistently
- [x] Output is generated from the process VMA list at read time
...
**Acceptance:**
- [x] `/proc/<pid>/fd/` lists one entry per open file descriptor
- [x] `readlink("/proc/<pid>/fd/<n>")` returns a stable target string for that FD
- [x] Closed FD slots are omitted from the directory listing
- [x] Pipe, socket, PTY, and device FDs use readable synthetic targets when no real path exists
...
**Acceptance:**
- [x] `Process` stores the executable path separately from `cmdline`
- [x] `sys_execve` records the resolved executable path when loading a new image
- [x] `readlink("/proc/<pid>/exe")` returns the process executable path when known
- [x] `readlink("/proc/self/exe")` resolves through `/proc/self` to the current executable path
- [x] `/proc/self` resolves to `/proc/<pid>/` using `current_pid()`
- [x] Works with all per-PID procfs files
...
**Acceptance:**
- [x] Reading `/proc/meminfo` returns MemTotal, MemFree, MemAvailable lines
- [x] Values sourced from the frame allocator statistics
- [x] Output in kB units matching Linux format
- [x] Reading `/proc/stat` returns at least `cpu` aggregate counters and boot count lines
- [x] CPU counters are derived from the scheduler/tick accounting available in the kernel
...
**Acceptance:**
- [x] `/proc/uptime` returns `<seconds_since_boot> 0.00` (idle time stubbed)
- [x] `/proc/version` returns `m3os version <version>` string
- [x] Uptime sourced from the tick counter or RTC
- [x] `/proc/mounts` lists each mounted filesystem with device, mount point, and type
- [x] Includes ramdisk, tmpfs, ext2 mounted at `/`, and procfs entries
- [x] Format: `<device> <mountpoint> <type> <options> 0 0`
...
**Acceptance:**
- [x] `readdir("/proc/")` lists PID directories + `self`, `meminfo`, `uptime`, `version`, `mounts`
- [x] `readdir("/proc/<pid>/")` lists `status`, `cmdline`, `maps`, `exe`, `fd`
- [x] `readdir("/proc/<pid>/fd/")` lists one entry per open file descriptor
- [x] PIDs listed only for processes that exist in `PROCESS_TABLE`
- [x] Integrates with existing `sys_linux_getdents64()` dispatch
...
**Acceptance:**
- [x] `FdBackend::DevZero` variant added to `FdBackend` enum
- [x] `open("/dev/zero")` creates an FD with `DevZero` backend
- [x] Read fills buffer with zero bytes, returns requested count
- [x] Write discards data, returns requested count
- [x] `fstat` returns character device mode (`0x2000 | 0o666`)
...
**Acceptance:**
- [x] `FdBackend::DevUrandom` variant added to `FdBackend` enum
- [x] `open("/dev/urandom")` creates an FD with `DevUrandom` backend
- [x] Read fills buffer with PRNG bytes (reuse `sys_getrandom` logic)
- [x] Write discards data (accepted but ignored, like Linux)
- [x] `fstat` returns character device mode (`0x2000 | 0o666`)
...
**Acceptance:**
- [x] `FdBackend::DevFull` variant added to `FdBackend` enum
- [x] `open("/dev/full")` creates an FD with `DevFull` backend
- [x] Read fills buffer with zero bytes (same as `/dev/zero`)
- [x] Write returns `NEG_ENOSPC`
- [x] `fstat` returns character device mode (`0x2000 | 0o666`)
...
**Acceptance:**
- [x] `open("/dev/zero")` returns an FD with `DevZero` backend
- [x] `open("/dev/urandom")` returns an FD with `DevUrandom` backend
- [x] `open("/dev/full")` returns an FD with `DevFull` backend
- [x] Existing `/dev/null` and `/dev/ttyN` behavior unchanged
- [x] Device nodes appear in `sys_linux_fstatat()` for `/dev/*` paths
...
**Acceptance:**
- [x] `sys_linux_read` match arm for `DevZero` fills buffer with 0x00
- [x] `sys_linux_read` match arm for `DevUrandom` fills buffer with PRNG bytes
- [x] `sys_linux_read` match arm for `DevFull` fills buffer with 0x00
- [x] `sys_linux_write` match arm for `DevZero` returns count (discard)
- [x] `sys_linux_write` match arm for `DevUrandom` returns count (discard)
- [x] `sys_linux_write` match arm for `DevFull` returns `NEG_ENOSPC`
...
**Acceptance:**
- [x] `Statfs` struct defined matching Linux x86_64 layout (120 bytes)
- [x] Fields: f_type, f_bsize, f_blocks, f_bfree, f_bavail, f_files, f_ffree, f_fsid, f_namelen, f_frsize
...
**Acceptance:**
- [x] Syscall 137 dispatches to `sys_statfs(path_ptr, buf_ptr)`
- [x] Syscall 138 dispatches to `sys_fstatfs(fd, buf_ptr)`
- [x] Returns correct `f_type` for each filesystem (tmpfs, ext2, procfs, ramdisk)
- [x] `f_bsize` and `f_frsize` set to 4096 (block size)
- [x] `f_blocks`/`f_bfree`/`f_bavail` reflect actual ext2 usage when available
- [x] tmpfs and procfs return reasonable defaults (e.g., large f_bavail)
- [x] Writes the `Statfs` struct to userspace buffer
...
**Acceptance:**
- [x] `ln <target> <link>` creates a hard link
- [x] `ln -s <target> <link>` creates a symbolic link
- [x] Prints error messages on failure (EEXIST, EXDEV, EPERM, etc.)
- [x] Binary added to initrd via xtask build
...
**Acceptance:**
- [x] `readlink <path>` prints symlink target to stdout
- [x] Returns exit code 1 if path is not a symlink
- [x] Binary added to initrd via xtask build
...
**Acceptance:**
- [x] `ls -l` shows `l` type prefix for symlinks
- [x] `ls -l` shows ` -> <target>` suffix for symlinks
- [x] Hard link count displayed for all files (from stat `st_nlink`)
...
**Acceptance:**
- [x] `stat` shows "symbolic link" type and target for symlinks
- [x] `stat` shows link count (nlinks) for all files
- [x] `stat` shows "character device" type for `/dev/*` nodes
...
**Acceptance:**
- [x] `ln` and `readlink` binaries compiled and placed in `kernel/initrd/`
- [x] Init process or shell can execute `/bin/ln` and `/bin/readlink`
...
**Acceptance:**
- [x] `symlink("/bin/sh0", "/tmp/mysh")` creates a working symlink
- [x] `readlink("/tmp/mysh")` returns `"/bin/sh0"`
- [x] Executing `/tmp/mysh` launches `sh0` via symlink resolution
- [x] `lstat("/tmp/mysh")` returns symlink metadata (not target metadata)
- [x] `stat("/tmp/mysh")` follows the symlink and returns target metadata
...
**Acceptance:**
- [x] Chain A -> B -> C resolves correctly to C's target
- [x] Circular symlink (A -> B -> A) returns `NEG_ELOOP`
- [x] Self-referencing symlink returns `NEG_ELOOP`
- [x] Resolution stops after 40 hops
...
**Acceptance:**
- [x] Creating a hard link increases the inode's `links_count` to 2
- [x] Both paths return identical `stat` results (same inode number)
- [x] Unlinking one path leaves the file accessible via the other
- [x] Unlinking the last path frees the inode and data blocks
- [x] `link()` on a directory returns `NEG_EPERM`
...
**Acceptance:**
- [x] Non-root user cannot open a 0600-mode file owned by another user
- [x] Non-root user cannot execute a file without the execute bit
- [x] Root user can open any file regardless of permissions
- [x] Non-root user cannot create files in a directory without write permission
- [x] `umask(077)` causes `mkdir()` to create directories with mode 0700
...
**Acceptance:**
- [x] `cat /proc/self/status` shows the cat process's own PID and name
- [x] `readlink /proc/self/exe` returns the running binary path
- [x] `cat /proc/meminfo` shows MemTotal, MemFree lines with non-zero values
- [x] `cat /proc/self/maps` shows mapped regions with permission flags
- [x] `ls /proc/self/fd` lists open descriptors and `readlink /proc/self/fd/1` resolves
- [x] `cat /proc/stat` shows aggregate CPU counters
- [x] `cat /proc/uptime` returns a reasonable uptime value
- [x] `cat /proc/version` returns the kernel version string
- [x] `ls /proc/` lists PID directories for running processes
...
**Acceptance:**
- [x] `echo test > /dev/null` succeeds silently
- [x] Reading 4096 bytes from `/dev/zero` returns all zero bytes
- [x] Reading from `/dev/urandom` returns non-zero data
- [x] Writing to `/dev/full` returns `ENOSPC`
...
- [x] Existing shell, pipe, socket, and filesystem workloads function correctly
...
**Acceptance:**
- [x] Phase 38 design doc status updated to "Complete"
- [x] Task list status updated to "Complete"
- [x] Roadmap README row updated from "Planned" to "Complete"
- [x] Task index README row updated from "Planned" to "Complete"

...
|---|---|---|---|
| A | Host toolchain setup and hello-rust | — | ✅ Done |
| B | Missing syscall support for Rust std | A | ✅ Done |
| C | Demonstration programs | A, B | ✅ Done |
| D | xtask integration for musl Rust binaries | A | ✅ Done |
| E | Custom m3os target specification (no_std) | A | ✅ Done |
| F | Integration testing and documentation | A–E | ✅ Done |
...
**Acceptance:**
- [x] `rustup target add x86_64-unknown-linux-musl` succeeds on the host
- [x] `cargo build --target x86_64-unknown-linux-musl --release` produces a static ELF binary
- [x] `file` confirms the output is `ELF 64-bit LSB executable, x86-64, statically linked`
...
**Acceptance:**
- [x] `userspace/hello-rust/` exists as a standard Rust binary crate (uses `std`)
- [x] `main()` prints `Hello from Rust on m3OS!` to stdout and exits cleanly
- [x] Cross-compiles with `cargo build --target x86_64-unknown-linux-musl --release`
- [x] Produces a statically-linked ELF binary under `target/x86_64-unknown-linux-musl/release/hello-rust`
...
**Acceptance:**
- [x] `hello-rust` binary is manually copied to the initrd for testing
- [x] Running `hello-rust` from the m3OS shell prints `Hello from Rust on m3OS!`
- [x] Process exits with code 0 (no crash, no hang)
- [x] Any missing syscalls encountered are documented for Track B
...
**Acceptance:**
- [x] Run `hello-rust` with serial logging enabled for unhandled syscall numbers
- [x] Each unhandled startup syscall is listed with its number, arguments, and expected behavior
- [x] Stubs or implementations are added so musl CRT startup completes without error
...
**Acceptance:**
- [x] `std::fs::read_to_string()` works on an existing file
- [x] `std::fs::write()` creates and writes a file
- [x] `std::fs::read_dir()` enumerates directory entries
- [x] `std::fs::metadata()` returns correct file size and type
...
**Acceptance:**
- [x] `TcpListener::bind("0.0.0.0:8080")` succeeds
- [x] `TcpStream::connect()` establishes a connection
- [x] `std::net::UdpSocket::bind()` and `send_to`/`recv_from` work
- [x] Socket options like `SO_REUSEADDR` are accepted (even if no-op)
...
**Acceptance:**
- [x] `std::io::stdin().read_line()` blocks and returns a line from terminal input
- [x] `std::io::stdout().write_all()` outputs text to the terminal
- [x] `print!` and `println!` macros work correctly
- [x] Programs reading stdin detect EOF correctly
...
**Acceptance:**
- [x] Reads and displays system information (memory usage, uptime, or similar)
- [x] Uses `std::fs::read_to_string()` to read files
- [x] Cross-compiles with `--target x86_64-unknown-linux-musl`
- [x] Runs inside m3OS and produces meaningful output
...
**Acceptance:**
- [x] Listens on a configurable port (default 8080)
- [x] Serves a static "Hello from Rust on m3OS!" HTML page
- [x] Handles multiple sequential HTTP requests
- [x] Responds with correct HTTP/1.1 headers (Content-Length, Content-Type)
- [x] Cross-compiles and runs inside m3OS
...
**Acceptance:**
- [x] Reads arithmetic expressions from stdin (e.g., `2 + 3 * 4`)
- [x] Evaluates and prints results with correct operator precedence
- [x] Handles integer and floating-point arithmetic
- [x] Prints a prompt and loops until EOF or `quit`
- [x] Cross-compiles and runs interactively inside m3OS
...
**Acceptance:**
- [x] `todo add "Buy milk"` appends to a todo file
- [x] `todo list` reads and displays all todos
- [x] `todo done 1` marks a todo as complete
- [x] Data persists across program invocations (stored in a file)
- [x] Cross-compiles and runs inside m3OS
...
**Acceptance:**
- [x] `build_musl_rust_bins()` compiles a configurable list of musl Rust crates
- [x] Uses `--target x86_64-unknown-linux-musl --release` with static linking flags
- [x] Copies built binaries to `kernel/initrd/`
- [x] Called from `build_kernel()` alongside `build_userspace_bins()` and `build_musl_bins()`
- [x] `cargo xtask image` produces a disk image containing the musl Rust binaries
...
**Acceptance:**
- [x] Built binaries are stripped with `strip` or `--strip` cargo flag
- [x] `Cargo.toml` profile for musl builds uses `opt-level = "z"` and `lto = true`
- [x] Final binary sizes are printed during the build for visibility
- [x] `hello-rust` binary is under 500 KB after stripping
...
**Acceptance:**
- [x] musl Rust crates compile without affecting the existing no_std build
- [x] `cargo xtask run` builds both no_std and musl Rust binaries in one invocation
- [x] No `-Zbuild-std` flag is passed for musl builds (musl target has prebuilt std)
- [x] Build artifacts for the two targets do not conflict
...
**Acceptance:**
- [x] `x86_64-m3os.json` exists with correct LLVM target triple, data layout, and ABI settings
- [x] `"os": "m3os"`, `"panic-strategy": "abort"`, `"disable-redzone": true`
- [x] `cargo build --target x86_64-m3os.json -Zbuild-std=core,alloc` compiles a trivial no_std binary
- [x] The produced binary is functionally equivalent to one built with `x86_64-unknown-none`
...
**Acceptance:**
- [x] Design doc updated to explain the two targets and their use cases
- [x] Decision documented: whether existing no_std crates migrate to the new target or stay on `x86_64-unknown-none`
- [x] Any differences in codegen, linking, or behavior between the two targets are noted
...
**Acceptance:**
- [x] `cargo xtask run` builds and packages all musl Rust binaries
- [x] `hello-rust` prints its message and exits
- [x] At least 3 of the 4 demo programs (sysinfo, httpd, calc, todo) run correctly
- [x] No regressions in existing no_std userspace binaries or C programs
...
**Acceptance:**
- [x] Design doc updated with final implementation details and status set to `Complete`
- [x] README row updated with task list link and status
- [x] CLAUDE.md updated with new musl Rust crates and documentation references
- [x] A "How to add a new Rust program" section exists in the design doc or a companion doc

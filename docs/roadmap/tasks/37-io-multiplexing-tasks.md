...
|---|---|---|---|
| A | Non-blocking I/O infrastructure | — | Planned |
| B | Non-blocking FD backends | A | Planned |
| C | Per-FD wait queues | — | Planned |
| D | Improved poll() | A, C | Planned |
| E | select() syscall | D | Planned |
| F | epoll interface | C, D | Planned |
| G | accept4() syscall | A | Planned |
...
**Acceptance:**
- [ ] `FdEntry` has a `nonblock: bool` field, defaulting to `false`
- [ ] `fork()` and `dup()` propagate the flag to the new FD entry
- [ ] `FdEntry` creation sites compile and pass `cargo xtask check`
...
**Acceptance:**
- [ ] `F_GETFL` returns `O_NONBLOCK` (0x800) if `nonblock` is set, 0 otherwise
- [ ] `F_SETFL` with `O_NONBLOCK` sets `nonblock = true` on the FD
- [ ] `F_SETFL` without `O_NONBLOCK` clears `nonblock = false`
- [ ] Existing `F_DUPFD`, `F_GETFD`, `F_SETFD` behavior unchanged
...
**Acceptance:**
- [ ] `socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0)` creates an FD with `nonblock = true`
- [ ] `socket()` without `SOCK_NONBLOCK` creates an FD with `nonblock = false`
...
**Acceptance:**
- [ ] Non-blocking read on an empty pipe returns `-EAGAIN` immediately
- [ ] Non-blocking write on a full pipe returns `-EAGAIN` immediately
- [ ] Blocking pipe read/write still works (yield loop) when `nonblock = false`
- [ ] Broken-pipe `EPIPE`/`SIGPIPE` behavior unchanged
...
**Acceptance:**
- [ ] Non-blocking TCP recv with no data returns `-EAGAIN`
- [ ] Non-blocking UDP recvfrom with no datagram returns `-EAGAIN`
- [ ] Non-blocking send on a full buffer returns `-EAGAIN`
- [ ] `MSG_DONTWAIT` flag still works independently of the FD's nonblock setting
...
**Acceptance:**
- [ ] Non-blocking read on PTY master with empty s2m buffer returns `-EAGAIN`
- [ ] Non-blocking read on PTY slave with empty m2s buffer returns `-EAGAIN`
- [ ] Non-blocking write on a full PTY ring buffer returns `-EAGAIN`
- [ ] Blocking PTY I/O unchanged when `nonblock = false`
...
**Acceptance:**
- [ ] Non-blocking read on stdin with no pending input returns `-EAGAIN`
- [ ] Non-blocking read on a `DeviceTTY` fd with no input returns `-EAGAIN`
- [ ] Blocking stdin read unchanged when `nonblock = false`
...
**Acceptance:**
- [ ] Each pipe slot has an associated `WaitQueue`
- [ ] `pipe_write()` calls `wake_all()` on the pipe's wait queue after writing data
- [ ] `pipe_read()` calls `wake_all()` after consuming data (wakes write-blocked pollers)
- [ ] Writer/reader close wakes the wait queue (EOF / broken pipe notification)
...
**Acceptance:**
- [ ] Each socket slot has an associated `WaitQueue`
- [ ] TCP data arrival wakes the socket's wait queue
- [ ] TCP connection establishment wakes the listening socket's wait queue
- [ ] Socket close/shutdown wakes the wait queue
- [ ] UDP datagram arrival wakes the socket's wait queue
...
**Acceptance:**
- [ ] Each PTY pair has two wait queues: one for master waiters, one for slave waiters
- [ ] Write to PTY master wakes slave waiters; write to PTY slave wakes master waiters
- [ ] PTY close wakes both wait queues
...
**Acceptance:**
- [ ] `stdin` module has a `WaitQueue`
- [ ] `push_char()` calls `wake_all()` after enqueuing input
- [ ] `signal_eof()` calls `wake_all()`
...
**Acceptance:**
- [ ] `fd_poll_events(fd_entry) -> u16` returns the current event mask for any FD type
- [ ] Covers PipeRead, PipeWrite, Socket (TCP/UDP), PtyMaster, PtySlave, Stdin, DeviceTTY
- [ ] Returns `POLLHUP` when the remote end is closed (broken pipe, TCP FIN)
- [ ] Returns `POLLERR` on error conditions (TCP RST)
...
**Acceptance:**
- [ ] `fd_register_waiter(fd_entry)` registers the current task on the FD's wait queue
- [ ] Supports all pollable FD backends (pipe, socket, PTY, stdin/TTY)
- [ ] Returns a handle or list for later deregistration
- [ ] Non-pollable FD types (Ramdisk, Tmpfs, DevNull) are skipped
...
**Acceptance:**
- [ ] First scan: if any FD is ready, return immediately (fast path preserved)
- [ ] No FDs ready: register on all wait queues and block via `WaitQueue::sleep()`
- [ ] Wakeup: re-scan all FDs and return the ready count
- [ ] Timeout of 0: non-blocking scan only (existing behavior)
- [ ] Timeout of -1: block indefinitely
- [ ] Positive timeout: block with timeout (yield-loop-with-timer if no kernel timer API)
- [ ] Deregister from all wait queues before returning
- [ ] CPU usage near zero while blocked (no spin)
- [ ] Existing `telnetd` works without regression
...
**Acceptance:**
- [ ] Syscall 23 dispatches to `sys_select()`
- [ ] Reads `fd_set` bitmaps from userspace for read, write, and except sets
- [ ] Checks readiness of each set bit using `fd_poll_events()`
- [ ] Blocks using the same wait-queue mechanism as `poll()` when no FDs are ready
- [ ] Returns the total number of ready FDs
- [ ] Writes modified `fd_set` bitmaps back to userspace (only ready bits set)
- [ ] `NULL` fd_set pointer means "don't check this set"
- [ ] `nfds` > `MAX_FDS` is clamped to `MAX_FDS`
...
**Acceptance:**
- [ ] Syscall 270 dispatches to `sys_pselect6()`
- [ ] Handles `timespec` timeout (seconds + nanoseconds)
- [ ] Signal mask argument accepted but not applied (signals not yet masked)
- [ ] Internally reuses the `select()` implementation
...
**Acceptance:**
- [ ] `EpollInstance` struct with interest list, ready list, and wait queue
- [ ] Global `EPOLL_TABLE` with `Mutex` protection (max 16 instances)
- [ ] `FdBackend::Epoll { instance_id: usize }` variant added to `FdBackend` enum
- [ ] epoll FDs are closeable — closing frees the instance
...
**Acceptance:**
- [ ] Syscall 291 dispatches to `sys_epoll_create1()`
- [ ] Allocates an `EpollInstance` and installs it as a new FD
- [ ] `EPOLL_CLOEXEC` flag sets `cloexec = true` on the FD
- [ ] Returns the new FD number, or `-EMFILE` if FD table is full
- [ ] Returns `-ENOMEM` if epoll table is full
...
**Acceptance:**
- [ ] Syscall 233 dispatches to `sys_epoll_ctl()`
- [ ] `EPOLL_CTL_ADD` (1): adds FD with requested events to interest list
- [ ] `EPOLL_CTL_MOD` (3): updates events for an already-registered FD
- [ ] `EPOLL_CTL_DEL` (2): removes FD from interest list
- [ ] Returns `-EEXIST` for duplicate ADD, `-ENOENT` for MOD/DEL on unregistered FD
- [ ] Reads `epoll_event` struct from userspace (events: u32, data: u64)
- [ ] Supports `EPOLLIN`, `EPOLLOUT`, `EPOLLHUP`, `EPOLLERR` event flags
...
**Acceptance:**
- [ ] Syscall 232 dispatches to `sys_epoll_wait()`
- [ ] Scans interest list for ready FDs (level-triggered: check current readiness)
- [ ] If ready events found, copies up to `maxevents` to userspace and returns count
- [ ] If no ready events, blocks on the epoll instance's wait queue
- [ ] Timeout of 0: non-blocking scan only
- [ ] Timeout of -1: block indefinitely
- [ ] Positive timeout: block with timeout
- [ ] Returns `-EINVAL` for `maxevents <= 0`
...
**Acceptance:**
- [ ] Pipe write/close wakes any epoll instance monitoring that pipe
- [ ] Socket data arrival / connection / close wakes epoll
- [ ] PTY data transfer wakes epoll
- [ ] stdin input wakes epoll
- [ ] Wakeup is efficient — only epoll instances monitoring the specific FD are woken
...
**Acceptance:**
- [ ] Syscall 288 dispatches to `sys_accept4()`
- [ ] Reuses the existing `sys_accept()` logic for connection acceptance
- [ ] `SOCK_NONBLOCK` flag sets `nonblock = true` on the accepted FD
- [ ] `SOCK_CLOEXEC` flag sets `cloexec = true` on the accepted FD
- [ ] Flags of 0 behaves identically to `accept()`
...
**Acceptance:**
- [ ] `poll()` on an idle pipe does not consume CPU (no yield loop)
- [ ] Data written to a pipe immediately wakes a poll waiter
- [ ] `telnetd` runs with improved poll — no regression in remote shell behavior
- [ ] Multi-FD poll wakes when any one of the monitored FDs becomes ready
...
**Acceptance:**
- [ ] `fcntl(fd, F_SETFL, O_NONBLOCK)` followed by read on empty pipe returns `-EAGAIN`
- [ ] `fcntl(fd, F_GETFL)` returns `O_NONBLOCK` for non-blocking FDs
- [ ] `fcntl(fd, F_SETFL, 0)` clears non-blocking and read blocks again
- [ ] `SOCK_NONBLOCK` in `socket()` creates a non-blocking socket
...
**Acceptance:**
- [ ] Test creates an epoll instance, adds a listening socket
- [ ] Accepts connections and adds them to epoll
- [ ] Echoes data back on readable sockets
- [ ] Handles 10+ simultaneous connections without error
...
- [ ] Existing pipe, socket, and PTY workloads function correctly
...
**Acceptance:**
- [ ] Phase 37 design doc updated with completion status

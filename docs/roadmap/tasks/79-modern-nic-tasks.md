...
- [~] (hardware-present — full ping needs root+VFIO) The dev host **has** a real RTL8125 (`0b:00.0 [10ec:8125] rev 05`, RTL8125**B**). Non-destructively validated the driver's hardware assumptions against the actual chip (no root, no disruption): device ID `0x8125` (confirms the corrected ID, not `0x8161`); **Ethernet class `0x020000`** (validates the INTx-fix gate on real silicon); **BAR2 = 64-bit MMIO 64K** (driver maps `R8169_BAR_INDEX=2`, not the BAR0 I/O port); **INTx pin A → IRQ 39** (the INTx path the fix routes to exists on the real part); Linux binds its **`r8169`** driver to this `0x8125` (validates the r8125-reuses-r8169-HAL architecture, matching upstream `r8169_main.c`); firmware `rtl8125b-2_0.0.2` (validates the Track D.1 `rtl_nic` signed-PHY-firmware-load path — the real part loads exactly such a blob). Full end-to-end `ping` requires VFIO-passthrough of the host's **active default-route NIC** (`enp11s0`) — root-only, disruptive to host networking — and is provided as an operator runbook (`scripts/r8125-vfio-validate.md`), not run unattended.

...
| C | Realtek RTL8111/8168 + RTL8169 (hardware-only) | — | ✅ Complete (host-tested; link is bench-hardware) |
| D | Realtek RTL8125 2.5GbE (hardware-only) | C | ✅ Complete (host-tested; link is bench-hardware) |
...
- [ ] (hardware-only — no QEMU r8169 model, physical RTL8111/8168 card or VFIO passthrough required; not present on this bench) On a real card, `ping` succeeds; the TxPoll doorbell starts transmission. *Ring/reset/IRQ logic complete + host-tested; gated solely by physical hardware.*
...
- [ ] (hardware-only — physical RTL8111/8168 card required; not present on this bench) Soft reset succeeds: ChipCmd `RST` self-clears within a bounded poll. *Soft-reset predicate host-tested (`r8169::tests::soft_reset_poll_predicate`); gated solely by physical hardware.*
...
- [~] (run on real silicon via VFIO — bring-up incomplete) The dev host **has** a real RTL8125 (`0b:00.0 [10ec:8125] rev 05`, RTL8125**B**, firmware `rtl8125b-2`), so — after standing up a Wi-Fi fallback link to keep operator SSH alive — the driver was **run against the physical card** by VFIO-passing it into the m3OS guest (`scripts/r8125-vfio-validate.md`). Non-destructive pre-checks first confirmed driver assumptions on the real chip: device ID `0x8125` (corrected, not `0x8161`); **Ethernet class `0x02`** (validates the INTx-fix gate on real silicon); BAR2 = 64-bit MMIO; INTx pin A → IRQ 39; Linux binds **`r8169`** to `0x8125` (validates the r8125-reuses-r8169-HAL architecture); `rtl8125b-2` firmware (validates the `rtl_nic` firmware-load path). **The live VFIO run reached the real card** (kernel enumerates `00:03.0 10ec:8125`; `r8125_driver` launches) but did **not** complete bring-up across **three** runs: (a) run 1 — the driver claimed the card and **hung in `Nic::bring_up`**; (b) runs 2–3 — a **non-deterministic PCI-discovery miss** (`select_r8125` → "no r8125 device present"). A bounded discovery **retry** was added (committed) on the hypothesis of a transient concurrent-enumeration race, but run 3 (12× retry) **still missed the card** — so the miss is *not* a simple transient race; its root cause is deeper, most likely the kernel `sys_device_config_read` / `sys_device_pci_enumerate` path for a **VFIO-passed** device (a real-hardware/q35/KVM interaction QEMU's emulated devices don't exercise). The bring-up hang therefore remains **unlocalized** (discovery kept failing before reaching the in-`bring_up` markers). Conclusion: this is genuine **kernel-side real-hardware debugging**, not convergent via blind VFIO iteration (each cycle detaches the host's sole production NIC). Tracked as the Phase-79 → Phase-83 hardware bring-up handoff. *Driver code + host tests are complete; the device-ID/class/BAR/INTx/firmware/architecture assumptions are validated on the real chip; the end-to-end real-silicon datapath is open.*

...
- [~] (run on real silicon via VFIO — bring-up incomplete) The dev host **has** a real RTL8125 (`0b:00.0 [10ec:8125] rev 05`, RTL8125**B**, firmware `rtl8125b-2`), so — after standing up a Wi-Fi fallback link to keep operator SSH alive — the driver was **run against the physical card** by VFIO-passing it into the m3OS guest (`scripts/r8125-vfio-validate.md`). Non-destructive pre-checks first confirmed driver assumptions on the real chip: device ID `0x8125` (corrected, not `0x8161`); **Ethernet class `0x02`** (validates the INTx-fix gate on real silicon); BAR2 = 64-bit MMIO; INTx pin A → IRQ 39; Linux binds **`r8169`** to `0x8125` (validates the r8125-reuses-r8169-HAL architecture); `rtl8125b-2` firmware (validates the `rtl_nic` firmware-load path). **The live VFIO run reached the real card** (kernel enumerates `00:03.0 10ec:8125`; `r8125_driver` launches) but did **not** complete bring-up: two runs showed (a) the driver claiming the card and **hanging in `Nic::bring_up`**, and (b) a non-deterministic **PCI-discovery miss** (`select_r8125` → "no r8125 device present"). These are real-hardware / concurrency defects that **QEMU could never surface** (no r8125 model). Remaining work — fixing the discovery race + real-silicon bring-up hang — is iterative hardware bring-up (each cycle detaches the host's production NIC); tracked as the Phase-79 → Phase-83 hardware handoff. *The driver code + host tests are complete; this item is now "exercised on real hardware, bring-up debugging open" rather than "untested".*

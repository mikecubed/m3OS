...
**Acceptance:**
- [ ] `AudioRequest` covers `QueryCaps`, `OpenStream { format, rate, layout } -> stream_id`, `SubmitFrames { stream_id, grant_handle, offset, len }`, `Drain { stream_id }`, and `CloseStream { stream_id }`.
- [ ] `AudioResponse` distinguishes the `SubmitFrames` outcomes `Ack { frames_consumed }` / `WouldBlock` (ring full — retry) / `Err(AudioDriverError)`, plus `StreamOpened(stream_id)` and a `Caps` descriptor; a host test round-trips **every** variant — including `WouldBlock` — through `encode`→`decode` byte-for-byte (`kernel_core::driver_ipc::audio::tests::request_roundtrip`, `response_roundtrip`, `would_block_roundtrip`).
- [ ] The PCM payload is **not** an inline field of any message — `SubmitFrames` references a grant handle + offset/length into the granted region (A.2), enforced by the absence of any `&[u8]`/`Vec<u8>` sample field in the request enum (grep-verifiable).
- [ ] `QueryCaps`→`Caps` returns a fixed 48 kHz / 2 ch / 16-bit descriptor for 1.0 (rate negotiation is deferred — see Documentation Notes); the driver validates this against the codec's reported `SUPPORTED_PCM_RATES`/`SUPPORTED_STREAM_FORMATS` in C.1.
...
**Symbol:** `PcmSubmission` (wraps a Phase 74 `sys_page_grant_*` single-use grant + offset/len); the driver-side `recv_and_copy` that maps the grant, copies into the driver's own `DmaBuffer` (allocated via `sys_device_dma_alloc`, real IOVA in the driver's IOMMU domain), and releases the grant
**Why it matters:** AGENTS.md mandates "bulk data: page capability grants, never IPC payloads." A page-grant is a *single-use ownership transfer* (it unmaps the sender's pages) and the received frames are mapped into the driver's **CPU** page table only — Phase 74's `iommu_remap_grant` is a documented no-op for the receiver-holds-a-device-claim case, so the granted pages are **not** in the driver's VT-d/AMD-Vi domain. Copying into the driver's own `sys_device_dma_alloc` buffer preserves IOMMU isolation with no new kernel primitive, and matches what the in-process `Ac97Backend` already does (`submit_frames_inner` copies caller bytes into its own `DmaBuffer`). This is the audio analog of NVMe's per-request `payload_grant` / e1000's per-frame bulk grant. (A persistent shared ring is also possible via the existing `sys_shm_create`/`map`/`unmap`/`destroy`, `0x1018`–`0x101B`, `kernel/src/mm/shm.rs` — the primitive `display_server` uses — but the per-submission grant is chosen for 80a because it matches the in-tree audio path and needs no lifetime negotiation.)

**Acceptance:**
- [ ] Host test validates grant offset/length bounds (a `SubmitFrames` offset+len always lands inside the granted region; out-of-range is rejected) — `audio_pcm::tests::submission_bounds`.
- [ ] The driver allocates its hardware PCM buffer via its **own** `sys_device_dma_alloc` and programs that buffer's **IOVA** into the controller (HDA BDL / AC'97 BDL); the granted pages are never programmed as a device address (grep + log assertion).
- [ ] No sample byte is ever copied into an `AudioRequest` (verified by A.1's enum shape).
- [ ] The granted region is released after the per-submission copy so frames are not leaked (ties to A.6).

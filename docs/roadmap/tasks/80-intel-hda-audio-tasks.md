...
**Status:** Complete — 80a/80b/80c landed + CI-validated (`audio-smoke`/`bell-smoke`/`doom-audio-smoke`/`hda-smoke` green). Track F's AMD-codec-enablement follow-up has now **also landed** (a claim-gated `sys_device_config_write` syscall + Linux-correct reset timing: PCI D0 power-up, in-reset PLL-settle delay, AMD/ATI snoop write — all QEMU-green, AMD-gated paths no-op on QEMU). The only remaining open item is Track F's operator audible-output **re-run** on the physical dev laptop to confirm whether the ALC1220 now enumerates (hardware-only; code + runbook + capture doc landed).
**Source Ref:** phase-80
...
| 80c | F | Real-hardware bring-up on the dev laptop (hardware-only) | E | ⏳ VFIO run executed: controller bring-up validated on real AMD `1022:15e3` (GCAP/GCTL valid), 2 real bugs fixed (slot collision, reset hardening). **AMD-enablement follow-up landed:** `sys_device_config_write` syscall + Linux-correct bring-up (D0 power-up, in-reset PLL-settle delay, AMD snoop) — diagnosis corrected (snoop is DMA-coherency, not the enumeration fix). **Open:** operator VFIO re-run to confirm ALC1220 enumeration; if still `STATESTS=0`, codec block is SoC-power-gated under VFIO (platform limit, not driver bug) |
...
**Acceptance:** *(Real-hardware VFIO run executed 2026-06-01 on the dev host; AMD-enablement follow-up landed the same day — see `docs/research/hda-realtek-capture.md` for the captured serial + corrected diagnosis.)*
- [~] **Partial — controller bring-up validated on real AMD silicon; codec enumeration pending an operator re-run after the landed follow-up.** Via VFIO passthrough of `1022:15e3`, `hda_driver` claims the device, maps BAR0, and resets the controller — `GCAP=0x4401` (4 OSS/4 ISS, 64-bit) and `GCTL=0x1` (out of reset) confirm MMIO + reset work on real silicon. The first run saw `STATESTS=0x0000`; tracing Linux `snd_hda_intel` corrected the diagnosis (the AMD snoop write is DMA-coherency, **not** the enumeration fix) and identified the real gaps — an in-reset PLL-settle delay + D0 power. Those are now landed (below); a VFIO re-run is needed to confirm enumeration on the real silicon.
- [ ] **Blocked behind codec enumeration** — no codec → no stream → no audible output. The driver's stream path itself is QEMU-validated (non-silent WAV). If the re-run still shows `STATESTS=0`, the codec block is SoC-power-gated under VFIO (platform limit, not a driver bug — see capture doc "Remaining risk").
- [x] **AMD-codec-enablement follow-up landed (2026-06-01) — corrected diagnosis + Linux-correct ring-3 bring-up.** Added (all QEMU-`hda-smoke`-green, AMD/D0 paths no-op on QEMU's Intel device): (1) a claim-gated **`sys_device_config_write`** syscall (`0x1129`) — kernel handler + dispatch + `driver_runtime::pci_config_write`, validation/numbering host-tested in `kernel_core::device_host::{config_write,syscalls}`; (2) a **PCI D0 power-up** before reset via the PM capability (`kernel_core::device_host::pci_pm`, host-tested); (3) an **in-reset PLL-settle delay** (~600 µs, matching Linux `snd_hdac_bus_enter_link_reset`) — the highest-confidence enumeration fix; (4) the **AMD/ATI snoop write** (`kernel_core::hda::amd`, config `0x42`, AMD-gated) for coherent DMA once a codec is up.

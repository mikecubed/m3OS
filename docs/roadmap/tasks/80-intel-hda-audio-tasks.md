...
**Status:** Complete — 80a/80b/80c landed + CI-validated (`audio-smoke`/`bell-smoke`/`doom-audio-smoke`/`hda-smoke` green). Track F's AMD-codec-enablement follow-up **landed and was re-run on the real AMD `1022:15e3` controller (2026-06-02)**: a claim-gated `sys_device_config_write` syscall + Linux-correct bring-up (PCI D0 power-up, in-reset PLL-settle delay, AMD/ATI snoop). The re-run **proved the new syscall works on real silicon** (snoop write succeeds) and that the controller is already in D0 and fully up — **but `STATESTS=0x0000` persists**. The **leading hypothesis** (inferred by elimination, not proven — see `docs/research/hda-realtek-capture.md`) is codec-rail power/clock gating under VFIO passthrough of `10:00.6` alone (ACPI `_PS0` codec-rail enable not replayed in the guest); untried passthrough workarounds remain. This is **not a driver bug**; the driver-side work is complete. Audible output now requires **bare-metal** validation (environment choice, outside m3OS code).
**Source Ref:** phase-80
...
| 80c | F | Real-hardware bring-up on the dev laptop (hardware-only) | E | ✅ Driver-side complete; codec-enum blocked by VFIO platform limit. Two VFIO runs on real AMD `1022:15e3`: controller bring-up validated (GCAP/GCTL valid), 2 bugs fixed (slot collision, reset hardening), `sys_device_config_write` syscall + Linux-correct bring-up (D0 power-up, in-reset PLL-settle delay, AMD snoop) landed + **proven on real silicon**. Re-run (2026-06-02) confirms `STATESTS=0` persists with the controller already in D0 → ALC1220 block is SoC-power-gated under passthrough of `10:00.6` alone (not a driver bug). Audible output needs bare metal |
...
- [x] Under `-device intel-hda`, the driver **receives a real stream-completion interrupt**: the device-host allocator routes **audio class `0x04`** through INTx (like the Phase 79 Ethernet fix) in `kernel/src/syscall/device_host.rs` (the HDA driver drives the legacy `INTCTL`/`SDnSTS` model with no MSI-X cause routing); the driver subscribes + binds + `arm_interrupts` (INTCTL GIE + the output stream's IE), and `handle_irq` decodes `INTSTS` + clears `SDnSTS.BCIS` (host-tested decode). The HDA IRQ is routed (`routed legacy INTx line 11 to vector 0x62`) and the `hda-smoke` gate now **asserts the BCIS interrupt fires** (`guest/hda: stream-completion (BCIS) interrupt delivered` → serial `hda_driver: stream IRQ (BCIS cleared)`). `SDnLPIB` polling remains the authoritative completion/position path (deferred DPB); BCIS is also cleared on the poll path so the level-triggered INTx can never storm.
...
- AMD-enablement follow-up (2026-06-01): `kernel-core/src/device_host/{config_write,pci_pm,syscalls}.rs`, `kernel-core/src/hda/amd.rs` (host-tested pure logic); `kernel/src/syscall/device_host.rs` + `kernel/src/pci/mod.rs` + `kernel/src/arch/x86_64/syscall/mod.rs` (the `sys_device_config_write` syscall); `userspace/lib/driver_runtime/src/pci_enum.rs` (`pci_config_write`); `userspace/drivers/hda/src/{controller,main}.rs` (D0 power-up + PLL-settle delay + AMD snoop)
...
> **Hardware confirmed present + matched to the driver; remaining step is operator-only (root + human ears).** Read-only inspection of the build host (it *is* the dev laptop) confirms the exact target: AMD `1022:15e3` HDA controller at `0000:10:00.6` → **Realtek ALC1220** codec (MSI subsystem `1462:ed76`), in **isolated IOMMU group 31** (VFIO-safe), alongside the GPU's ATI HDMI codec (`10:00.1`) — the analog-vs-HDMI multi-codec case `select_codec` handles. The driver's design assumptions all match (`hda_pci_match` accepts `1022:15e3`; `select_codec` prefers the analog ALC1220; `realtek::*` drives the ALC1220 amp-enable). All *code* is landed + CI-validated against QEMU's generic codec; the runbook (`scripts/hda-vfio-validate.md`) + capture doc (`docs/research/hda-realtek-capture.md`, now with the real confirmed identifiers + a **turnkey command sequence for this exact host**) are landed. The remaining bullets require **root (sudo)** to bind vfio-pci + run QEMU, and a **human to listen** — both are by definition operator actions (this session has no passwordless sudo and cannot hear). Run the documented commands to complete them.

**Acceptance:** *(Two real-hardware VFIO runs on the dev host — 2026-06-01 initial + 2026-06-02 re-run after the follow-up landed. See `docs/research/hda-realtek-capture.md` for both captured serials + the corrected diagnosis.)*
- [~] **Controller bring-up validated on real AMD silicon; codec enumeration not reached under this VFIO config.** Via VFIO passthrough of `1022:15e3`, `hda_driver` claims the device, maps BAR0, forces/confirms D0, applies the AMD snoop write, and resets the controller — `GCAP=0x4401` and `GCTL=0x1` confirm MMIO + reset work on real silicon, and the snoop write proves `sys_device_config_write` end-to-end on hardware. **`STATESTS=0x0000` persists** across both runs even with the Linux-correct bring-up (D0 — read back as already-D0, so no force needed — + ~600 µs in-reset PLL-settle + 2 reset retries + 4 s poll each). The **leading hypothesis** (inferred by elimination, not proven — see `docs/research/hda-realtek-capture.md`) is codec-rail power/clock gating (ACPI `_PS0` not replayed in the guest under passthrough of `10:00.6` alone); untried passthrough workarounds remain. Driver-side complete; not a driver bug.
- [ ] **Audible output — needs bare metal, not reachable under this VFIO config.** No codec → no stream → no audible output through passthrough. The driver's stream path is QEMU-validated (non-silent WAV) and the controller bring-up is now Linux-correct, so native/bare-metal boot should enumerate. See capture doc "Path to audible output from here".
- [x] **AMD-codec-enablement follow-up landed (2026-06-01) + re-validated on real HW (2026-06-02) — corrected diagnosis + Linux-correct ring-3 bring-up.** Added (all QEMU-`hda-smoke`-green, AMD/D0 paths no-op on QEMU's Intel device): (1) a claim-gated **`sys_device_config_write`** syscall (`0x1129`) — kernel handler + dispatch + `driver_runtime::pci_config_write`, validation/numbering host-tested in `kernel_core::device_host::{config_write,syscalls}`, **proven working on real AMD silicon** (snoop write succeeds); (2) a **PCI D0 power-up** before reset via the PM capability (`kernel_core::device_host::pci_pm`, host-tested) — found the controller already in D0; (3) an **in-reset PLL-settle delay** (~600 µs, matching Linux `snd_hdac_bus_enter_link_reset`); (4) the **AMD/ATI snoop write** (`kernel_core::hda::amd`, config `0x42`, AMD-gated) for coherent DMA once a codec is up.
- [x] **Three real-hardware issues uncovered + recorded (F.1 #3 met):** (1) PCI-slot collision with the nvme sentinel — **fixed** (pin VFIO device to slot 0x8); (2) reset/codec-ready hardening — **fixed** (STATESTS-clear + post-CRST delay + reset retry + wall-clock STATESTS poll); (3) ALC1220 not enumerating via standard reset on the AMD controller — **diagnosed + follow-up landed** (config-write syscall + Linux-correct reset timing/power, above). All committed; captured in `docs/research/hda-realtek-capture.md`.

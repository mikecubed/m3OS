...
| 80c | F | Real-hardware bring-up on the dev laptop (hardware-only) | E | ⏳ VFIO run executed: controller bring-up validated on real AMD `1022:15e3` (GCAP/GCTL valid), 2 real bugs fixed (slot collision, reset hardening); **open:** ALC1220 codec enumeration needs AMD vendor-quirk + a config-write syscall (recorded) → audible output blocked behind it |
...
**Acceptance:** *(Real-hardware VFIO run executed 2026-06-01 on the dev host — see `docs/research/hda-realtek-capture.md` for the captured serial.)*
- [~] **Partial — controller bring-up validated on real AMD silicon; codec enumeration blocked by an AMD vendor quirk.** Via VFIO passthrough of `1022:15e3`, `hda_driver` claims the device, maps BAR0, and resets the controller — `GCAP=0x4401` (4 OSS/4 ISS, 64-bit) and `GCTL=0x1` (out of reset) confirm MMIO + reset work on real silicon. But `STATESTS=0x0000` after multiple resets + ~8 s polling: the ALC1220 does **not** assert SDI presence via the standard HDA `GCTL.CRST` sequence — real drivers bring AMD codecs up with vendor-specific snoop/link handling QEMU does not model (needs a PCI-config-write syscall, which m3OS lacks). Documented as follow-up.
- [ ] **Blocked behind the codec-enumeration item above** — no codec → no stream → no audible output. The driver's stream path itself is QEMU-validated (non-silent WAV).
- [x] **Three real-hardware issues uncovered + recorded (F.1 #3 met):** (1) PCI-slot collision with the nvme sentinel — **fixed** (pin VFIO device to slot 0x8); (2) reset/codec-ready hardening — **fixed** (STATESTS-clear + post-CRST delay + reset retry + wall-clock STATESTS poll); (3) ALC1220 not enumerating via standard reset on the AMD controller — **recorded** with the required follow-up (`sys_device_config_write` + AMD snoop/link config). All committed; captured in `docs/research/hda-realtek-capture.md`.

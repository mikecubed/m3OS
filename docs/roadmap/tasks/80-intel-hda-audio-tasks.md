...
**Status:** Complete — 80a/80b/80c landed + CI-validated (`audio-smoke`/`bell-smoke`/`doom-audio-smoke`/`hda-smoke` green). Track F's AMD-codec-enablement follow-up **landed and was re-run on the real AMD `1022:15e3` controller (2026-06-02)**: a claim-gated `sys_device_config_write` syscall + Linux-correct bring-up (PCI D0 power-up, in-reset PLL-settle delay, AMD/ATI snoop). The re-run **proved the new syscall works on real silicon** (snoop write succeeds) and that the controller is already in D0 and fully up — **but `STATESTS=0x0000` persists**, so the ALC1220 codec block is power/clock-gated at the SoC level under VFIO passthrough of `10:00.6` alone (ACPI `_PS0` codec-rail enable not replayed in the guest). This is a **VFIO/platform limitation, not a driver bug**; the driver-side work is complete. Audible output now requires **bare-metal** validation (environment choice, outside m3OS code).
**Source Ref:** phase-80
...
- [~] **Controller bring-up validated on real AMD silicon; codec enumeration confirmed blocked by a VFIO platform limit.** Via VFIO passthrough of `1022:15e3`, `hda_driver` claims the device, maps BAR0, forces/confirms D0, applies the AMD snoop write, and resets the controller — `GCAP=0x4401` and `GCTL=0x1` confirm MMIO + reset work on real silicon, and the snoop write proves `sys_device_config_write` end-to-end on hardware. **`STATESTS=0x0000` persists** across both runs even with the Linux-correct bring-up (D0 — read back as already-D0, so no force needed — + ~600 µs in-reset PLL-settle + 2 reset retries + 4 s poll each). The codec block is power/clock-gated at the SoC level (ACPI `_PS0` not replayed in the guest under passthrough of `10:00.6` alone). Driver-side complete; this is a platform/passthrough limitation, not a driver bug.

...
**The milestone works, to 100% with no in-scope deferrals: a USB keyboard AND mouse drive m3OS, and a typed USB key visibly renders at the shell prompt.** The `usb-smoke` gate (boots `qemu-xhci` + `usb-kbd` + `usb-mouse`) asserts, in causal order: `XHCI_BRINGUP:enable-slot:OK` → `XHCI_USB:server-ready` → `usb-hid: polling` → a focused term prompt → QMP `send-key a` decoded to `USB_HID:key kind=0 sym=0x00000061` → QMP relative mouse motion decoded to `USB_HID:mouse` (live mouse → `mouse_server`) → typed USB text rendering at the prompt (≥3 changed framebuffer rows via `screendump`).

**Closure pass — the pragmatic deltas from the first cut were all removed:**
- **`usb-hid` itself issues `SET_PROTOCOL(0)`/`SET_IDLE(0)`** via `UsbRequest::ControlRequest`, served by the xHCI server (`Controller::control_request` → `control_transfer`). This works inside the IRQ-bound recv loop because `notification::wait` drains the same `PENDING` word the bound `ipc_recv_msg` does, and the single-threaded server is never in both at once (verified: `kernel/src/ipc/notification.rs`). Endpoints arm lazily on the first `PollInterruptIn`.
- **Multi-slot controller.** `Controller.slot_ctx: Option<SlotContext>` → `slots: Vec<SlotContext>` keyed by `slot_id`; `scan_ports` returns all connected ports; `bring_up_controller` enumerates each into its own slot. A keyboard **and** mouse are both Configured and served — verified live (`usb-smoke` decodes both).
- **Live mouse + glyph-at-prompt.** `usb-smoke` attaches `usb-mouse`, injects motion via QMP `input-send-event`, and screendump-verifies typed USB text renders at the focused term (USB → usb-hid → kbd_server → display_server → term). The B.3 "serial-sentinel-not-sufficient" / design-doc "types into the prompt" criteria are now met literally.
- **HID reports return inline** via `ipc_store_reply_bulk`; the `PageGrant` bulk transport is retained for future bulk endpoints → **Phase 90**.

Remaining deferrals — **each assigned to [Phase 90 — USB Class Expansion](../90-usb-class-expansion.md)**: external-hub multi-tier enumeration (`usbhub` live consumer), live HID Report Protocol (A.3 skeleton stays skeleton per its own acceptance), USB hot-plug surface, USB mass storage / audio / video, multi-controller concurrent IRQ servicing, and `GetDescriptors`/`ConfigureEndpoints`/`SubmitTransfer`-over-IPC (reply `ENOSYS` today).

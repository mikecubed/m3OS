...
**Acceptance:**
- [ ] A client resolves `FindByHid("DLL0945")` → a device handle, `GetResources` → a `DeviceResources` (I2C addr + GpioInt), over IPC. *(`FindByHid`/`GetCrs`/`Sta` labels are served (path + raw `_CRS` bytes; decode client-side via `kernel-core`); the first real client lands with Phase 102. Phase 103 slice 1 extended the protocol with `ACPI_EVAL` (label 7) — evaluate a path, reply with the `aml::wire`-encoded `AmlValue` — consumed by `powerd`.)*

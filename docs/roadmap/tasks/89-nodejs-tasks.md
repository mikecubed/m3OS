...
**Acceptance:**
- [ ] A `timerfd` is a pollable fd: it becomes readable when the timer expires, a `read(2)` returns the `u64` expiration count and resets it, and it is selectable by `epoll_ctl`/`epoll_wait` (level- **and** edge-triggered, reusing the `EpollInterest.last_ready` watermark at `mod.rs:21103`).
- [ ] `CLOCK_MONOTONIC` and `CLOCK_REALTIME` are accepted; `TFD_NONBLOCK`/`TFD_CLOEXEC` creation flags and `TFD_TIMER_ABSTIME` are honored; one-shot **and** interval (`it_interval`) rearm both work.
- [ ] Host-tested expiry-accounting logic lives in `kernel-core` (e.g. `kernel_core::timerfd`) with unit tests for the expiration-count and rearm math, mirroring how `kernel_core::epoll::evaluate_interest` factors the epoll edge logic.
- [ ] The dispatch arms are added to the `mod nr {}` block (`mod.rs:1330`–`1432`) and the dispatch match (`mod.rs:1869`–`2038`) with the correct Linux syscall numbers.
...
**Acceptance:**
- [ ] **Decision recorded** in the task notes + learning doc: either (a) `signalfd4` is implemented as a pollable fd that dequeues `signalfd_siginfo` records for the masked signal set, or (b) the Node build is configured so libuv compiles its self-pipe + `sigaction` fallback, with a one-line justification.
- [ ] If (a): a `signalfd4` fd is readable when a masked signal is pending, `read` returns the 128-byte `signalfd_siginfo`, and it is epoll-selectable.
- [ ] If (b): `node -e` delivering `SIGINT`/`SIGTERM`/`SIGCHLD` to a handler still fires (`process.on('SIGINT', …)`), proving the self-pipe path works on the present `pipe2`+`rt_sigaction` substrate; the absence of `signalfd` is documented as a *supported configuration*, not a latent bug.
...
**Acceptance:**
- [ ] A note enumerates, with exact symbols, the already-present substrate Node reuses: `mprotect` RW→RX (`mod.rs:11620`,`:11644`), demand-fault per-page `PROT_EXEC` (`interrupts.rs:542`), `mmap` `MAP_FIXED`+`PROT_NONE` (`mod.rs:10863`), epoll/`epoll_pwait`/`eventfd2`/`pipe2`, `clone(CLONE_THREAD)` for the libuv threadpool + V8 background-compile threads, `SIGURG`/`tgkill` for async interrupts, `getrandom`/`AT_RANDOM` for the V8/OpenSSL entropy bootstrap.
- [ ] The existing `wx-violation` gate (`SMOKE:wx-violation:PASS`, `xtask/src/main.rs:7560`) is confirmed to cover the V8 commit pattern (mmap RW → write → `mprotect(R|X)` succeeds; `mprotect(W|X)` EINVAL) — and the Track C `node-smoke` running real JIT'd JS is the end-to-end proof on top of it.
- [ ] No `PROT_WRITE|PROT_EXEC` RWX mapping is ever requested by the running Node binary (verified by the absence of an mprotect-W|X EINVAL in the `node-smoke` serial log); if V8 attempts RWX, that is a B.2 build-config defect, not a kernel change.

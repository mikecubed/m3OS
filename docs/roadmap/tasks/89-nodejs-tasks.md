...
- [ ] **Opt-in live arm (`M3OS_NODE_NET=1`):** `https.get` cert-validate + `npm install is-number`. **Implemented and wired** behind `M3OS_NODE_NET`. The libuv-threadpool futex blocker is **resolved** (D.1) — node's `http.get` now completes the request cycle (proven always-on by `NODE_EGRESS_OK`). These arms stay opt-in because they need **real outbound internet** (example.com:443, registry.npmjs.org), which repo CI lacks — exactly like `git-https-smoke`'s `M3OS_GIT_HTTPS_NET`. *(Stays `[ ]`: the gate code is done; checking it off requires a one-off run in a network-enabled environment.)* When that run happens, `node -e "https.get('https://example.com/')"` validates the cert chain and `npm install is-number` over `registry.npmjs.org` should print `added 1 package`.

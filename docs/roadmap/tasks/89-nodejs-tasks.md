...
- [x] `cargo xtask port build node` produces a **sealed `target/pkgcache/<key>.m3pkg` (~130 MB, the heaviest artifact in the tree)**, and a second build is a **pure pkgcache hit (`PKGCACHE: hit`, "zero compiler invocations")**; `build_node_port()` + `"node"` in `BUILDABLE_PORTS` wire it into `port build all` + `port list`. (The content key folds the `M3OS_NODE_CLANG`/`M3OS_NODE_CLANGXX` host-clang identity, so a different cross-compiler does not serve a stale hit.)
...
**Acceptance:**
- [x] `docs/89-nodejs.md` exists, follows the aligned learning-doc template sections, and explains the V8 W^X/JIT model (jitless), the libuv `timerfd`/`signalfd` decisions, the static-musl build, and the npm path in learner-friendly terms. **As-built:** the sealed `.m3pkg` measures **~130 MB** (the heaviest artifact in the tree — WASM stays compiled in under jitless).

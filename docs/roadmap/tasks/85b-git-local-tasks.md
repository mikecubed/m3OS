...
**Acceptance:**
- [x] The gate establishes a commit identity via a bundled minimal `/etc/gitconfig` (`user.name`/`user.email` + `init.defaultBranch=main` + `core.pager=cat`), written into the data disk via `populate_ext2_files`. (Verified git resolves its system config to `/etc/gitconfig` for `prefix=/usr`.) No `safe.directory` override is bundled: the smoke runs as root and tmpfs creates `/tmp/gitsmoke`/`.git` owned by uid 0, so git's dubious-ownership guard never fires (a global `safe.directory=*` would only weaken safety for future multi-user use). The repo is created in tmpfs `/tmp/gitsmoke`; no extra `/usr/src` fixtures needed, so no `cargo xtask clean` step is required (the gate recreates the disk itself).

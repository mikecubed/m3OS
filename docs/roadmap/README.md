...
| 97 | `dlopen-test-smoke` Intermittent TCG Stall (debugging) | Root-cause + fix the intermittent `smoke-test` **step 26** (`dlopen-test-smoke`) failure under plain TCG. The test demand-pages ~5 small `.so`s via the Phase 95b `MAP_LAZY_FILE` blocking page-fault→`vfs_server` read, and intermittently either stalls (no sentinel within the wait) or only emits `PASS` after the window. The handoff's "blocking demand-read lost-wakeup" hypothesis is **falsified** (`readelf`: every DSO is ramdisk-embedded → synchronous in-kernel memcpy, no `vfs_server` IPC on this gate's path); the leading **surviving** suspect is the cross-core **TLB shootdown** `dlclose`'s `munmap` runs twice on the `FINI`→`PASS` path (`unmap_dso`→`sys_munmap`→`tlb_shootdown_range`) under `-smp 4` TCG oversubscription. Observability-first: confirm before fixing, then replace the observability-blind PASS-or-SKIP gate. Surfaced while landing the stat-identity fix (`ae01ed4`), which made steps 1–25 pass so the runner now reaches step 26. | **In Progress** — design + task docs landed; mitigation only so far (`dlopen-test-smoke` wait widened 30s→120s in `ae01ed4`); pre-existing (fails on `origin/main`); slow-VFS is largely a TCG artifact (passes under KVM) | `phase-97` | [Phase 97](./97-dlopen-smoke-tcg-stall.md) | [Tasks](./tasks/97-dlopen-smoke-tcg-stall-tasks.md) |
| 98 | Roadmap Audit & Re-Charter (toward a real-hardware GUI workstation) | End-of-roadmap inflection: **(A)** an evidence-backed **audit** reconciling every completed phase's Status against a passing gate / recorded HW run / host test (Phase 96's stale `Planned` while HW-validated shows the drift this catches), producing a per-phase **Validated / Claimed-unvalidated / Regressed** verdict; **(B)** a **re-chartered roadmap** for the next arc — a usable **GUI workstation on the real Tiger Lake laptop**: GUI session bring-up on the now-write-combining framebuffer, the **I2C-HID touchpad** driver (the GUI pointer — Intel LPSS DesignWare I2C + `imt`/`dwiic`; the laptop has no PS/2 pointer), input polish (USB-kbd text mode, the `usb-hid`/`usbhub` CPU-hog), and **Intel AX201/CNVi Wi-Fi** (`iwx`, the only built-in NIC). Folds in Phase 97 + the Phase 96 *Deferred* items. | **Proposed** | `phase-98` | [Phase 98](./98-roadmap-audit-and-recharter.md) | — |

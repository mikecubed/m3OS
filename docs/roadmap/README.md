...
| 76b | Dynamic Linker: DT_NEEDED + Relocations + Constructors | Real `_dlstart` self-relocation, `PT_DYNAMIC` parse + dependency graph, x86_64 relocations (`GLOB_DAT`/`JUMP_SLOT`/`RELATIVE`/`64`), `DT_INIT`/`DT_INIT_ARRAY` constructors, `xtask::build_shared_lib`, `libhello.so` + `dynlink_hello` end-to-end | Planned | `phase-76b` | [Phase 76b](./76b-dynamic-linker-bringup.md) | [Tasks](./tasks/76b-dynamic-linker-bringup-tasks.md) |

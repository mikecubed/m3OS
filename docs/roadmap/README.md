...
| Phase | Theme | Primary Outcome | Status | Source Ref | Milestone | Tasks |
|---|---|---|---|---|---|---|
| 77 | Pre-1.0 Correctness, Cheap Security, and Network Polish | Bundle phase: SSH disconnect hang + futex CHILD_CLEARTID lost-wakeup, `sys_nanosleep` cleanup, SMEP+SMAP, `PT_TLS` + multithread TLS, DNS resolver wiring, RFC 6298 TCP retransmit + 64-conn lift, AMD microcode loading, `epoll_*` verified, `/proc` htop/ps fix, 5 handoffs resolved | Complete | `phase-77` | [Phase 77](./77-pre-1-0-cleanup.md) | [Tasks](./tasks/77-pre-1-0-cleanup-tasks.md) |

...
| 68 | Phase 56 Display Server Closeout | Subscription event push (4 existing + 2 new event kinds), compositor damage tracking, `ModifierSide` field with global `PROTOCOL_VERSION` 1→2 bump, extracted init `manifest.rs`/`supervisor.rs` with multi-service `depends=` and `on-restart=` directive, `mouse_server` as managed service depending on `kbd_server`. Closes audit § C5 | Planned | `phase-68` | [Phase 68](./68-display-server-closeout.md) | [Tasks](./tasks/68-display-server-closeout-tasks.md) |

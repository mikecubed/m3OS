...
| 76d | Dynamic Linker: PLT Lazy + GNU Hash + Versioning | `_dl_runtime_resolve` asm trampoline + GOT[1]/GOT[2] install + lazy `JUMP_SLOT` rebase; `DT_GNU_HASH` Bloom-filter + bucket + chain lookup with dispatcher preferring GNU over SysV; `LD_BIND_NOW` env-var flag flips lazy default to eager; `dynlink-hello-gnu-smoke` end-to-end gate (default + LD_BIND_NOW=1 + W^X assertion via `/proc/self/maps`); `DT_VERSYM` / `DT_VERDEF` / `DT_VERNEED` parser in new `ldso_core::ver` module (4 host tests). `D2.2` runtime version-aware lookup, `D2.3` strict mode under LD_BIND_NOW, and the `dynlink-hello-versioned-smoke` gate (G) are scoped to a 76d follow-up. Kernel bumped to `0.76.3`. | **Substantially Complete** (S1, D1, B4, E4, F, D2.1 shipped; D2.2/D2.3 + G deferred) | `phase-76d` | [Phase 76d](./76d-dynamic-linker-polish.md) | [Tasks](./tasks/76d-dynamic-linker-polish-tasks.md) |

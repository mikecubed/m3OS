...
| 64 | Phase 57 Session Manager Lifecycle | Real `start/stop/restart` (replace unconditional-Ack stubs); SIGCHLD-based lifecycle; restart budget; text-fallback motion. Closes audit § B2 / F2 | In Progress | `phase-64` | [Phase 64](./64-session-manager-lifecycle.md) | [Tasks](./tasks/64-session-manager-lifecycle-tasks.md) |

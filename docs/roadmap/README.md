...
| 89 | Node.js | A statically-linked **jitless Node.js v22.22.3** (`--v8-options=--jitless`, static musl, sealed ~120 MB `.m3pkg`) runs natively inside m3OS; the local-runtime milestone is landed and the npm/TLS path is the Phase 90 dependency | 🟢 Local runtime landed (`node-smoke` PASSES under `M3OS_KVM=1`: boot `0.89.0` → `pkg install node` → `node --version` v22.22.3 → fs/process/event-loop/**timer** probe (NODE_TIMER_OK validates the A.1 `timerfd` end-to-end) → tls/dns/crypto load). Bonus kernel fix: `F_SETFD` now returns `EBADF` for closed fds (was silent-success → node's libuv CLOEXEC-all-fds loop busy-spun forever). **Networking is a tracked follow-up:** node's `http.get` deadlocks in libuv's threadpool on the DNS-resolve futex (`BlockedOnFutex "no waker registered"`; the host *does* see the TCP connect) and there is no 127.0.0.1 loopback — so egress/HTTPS/`npm install` are opt-in (`M3OS_NODE_NET`) pending the futex lost-wakeup fix. | `phase-89` | [Phase 89](./89-nodejs.md) | [Tasks](./tasks/89-nodejs-tasks.md) |

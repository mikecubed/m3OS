...
| 86c | Networking and GitHub: HTTPS/TLS + git smart-HTTP | `mbedtls` + `curl` ports (SIMD-off-safe C crypto; pure-Rust rustls deferred) + `git` rebuilt with `NO_CURL` removed + the Phase 85b absence-assertions inverted + X.509 chain/hostname verification against the 86a CA bundle + PAT credentials → `git clone`/push over HTTPS, with a **mandatory rejected-bad-cert** arm. Kernel `0.86.2`. | 🟡 Implemented (the mbedtls+curl+git HTTPS chain cross-builds and the inverted assertions pass — `curl_multi_perform`/`mbedtls_ssl_handshake` present, `SSL_CTX_new` absent; `git-https-smoke` core proves the chain installs + the static curl+mbedTLS runs on m3OS + trust is wired; the live bad-cert-reject + public `https://` clone are opt-in `M3OS_GIT_HTTPS_NET=1`) | `phase-86c` | [Phase 86c](./86c-https-git-transport.md) | [Tasks](./tasks/86c-https-git-transport-tasks.md) |

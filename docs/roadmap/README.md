...
| Phase | Theme | Primary Outcome | Status | Source Ref | Milestone | Tasks |
|---|---|---|---|---|---|---|
| 111 | Remote Debugging (Source-Level Kernel + Userspace) | Turn the long-deferred "gdb stub" item into real source-level debugging in three escalating tiers: **(A)** free in-emulator **kernel** debugging via QEMU's gdbstub + a DWARF-bearing build (`cargo xtask debug` → `-s -S`); **(B)** the trap/debug-register substrate that registers the absent-since-Phase-3 `#DB` handler, upgrades `#BP`, and adds `RFLAGS.TF` single-step + a `DR0`–`DR7` wrapper + `int3` patching; **(C)** an in-kernel `kgdb`-style GDB-RSP stub over **polled COM2** with NMI-IPI **SMP all-stop** + panic→stub hook, so the same workflow works **on bare metal**; and **(D)** a `ptrace`-backed userspace debugger — generate the defined-but-unused `SIGTRAP`, convert the kill-on-trap path to **stop-and-notify**, add `sys_ptrace`, and an `m3gdbserver` so host `gdb` debugs ring-3 programs over TCP. `kgdb`/`ptrace` are build-time features, off in production (W^X/PKU/capability posture). | In progress — **Track A landed**: `[profile.kdebug]` (DWARF, no-LTO) + `cargo xtask debug` launch QEMU halted (`-s -S`) with an auto-generated gdb script; the RSP round-trip (breakpoint hit at `kernel_main_entry`) is validated. Found + handled a charter correction — the kernel is a PIE relocated by a fixed 1 TiB offset, so `add-symbol-file -o 0x10000000000` is required. **Track B landed**: registers the absent-since-Phase-3 `#DB` handler + upgrades `#BP` to a dispatcher (RIP-fixup), `RFLAGS.TF` single-step, a `DebugRegs` `DR0`–`DR7` wrapper (bits host-tested in `kernel_core::debug_regs`), and an `int3` patch primitive, with a ring-0/ring-3 dispatch seam for C/D and a safe production default; `debug-substrate-smoke` (`M3OS_DEBUG_SUBSTRATE_REGRESSION`) PASS. **Track C.1 (RSP codec) landed**: `kernel_core::gdb_rsp` (packet framing/checksum/ack-nak/RLE/hex, 11 host tests) — the wire-format foundation for the in-kernel stub. C.2–C.5 (stub loop + polled COM2 + SMP all-stop + gate) and Track D (ptrace + m3gdbserver) planned. | `phase-111` | [Phase 111](./111-remote-debugging.md) | [Tasks](./tasks/111-remote-debugging-tasks.md) |

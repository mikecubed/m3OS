...
flowchart TD
    ...
    P105 --> P112
    P90 -.->|92b Report-protocol wheel| P112
...
| 102 | I2C-HID Touchpad (Intel LPSS) | The real built-in pointer (the laptop has no PS/2 pointer): an Intel LPSS DesignWare I2C controller (`dwiic` ref) + I2C-HID transport + multitouch report parse (reusing the Phase 92b HID Report-Protocol decode) → `mouse_server` inject, replacing the Phase 100 interim USB mouse. Depends on 101. | Planned | `phase-102` | [Phase 102](./102-i2c-hid-touchpad.md) | [Tasks](./tasks/102-i2c-hid-touchpad-tasks.md) |
...
| Phase | Theme | Primary Outcome | Status | Source Ref | Milestone | Tasks |
|---|---|---|---|---|---|---|
| 112 | Terminal Daily-Driver Polish | `term`'s 1000-line scrollback ring is now **viewable** — a `Screen::view_offset` viewport composited by `compose_view`, bound to Shift+PageUp/PageDown/Home/End on every lane and to the mouse wheel on Report-protocol pointer lanes, with snap-to-bottom on output and on any key that writes PTY bytes. `InputHandler::translate` gained a typed `KeyOutcome` return channel, which also let the **unshifted** PageUp/PageDown/Home/End VT sequences `term` never had (plus a missing Delete → `ESC[3~`) be filled in from the `hid_poll` table and pinned to it by a whole-keysym-block range sweep. **Mouse text selection + compositor-brokered copy/paste** landed on top: linear/Alt-block selection with an inverted highlight, copy-on-release and Ctrl+Shift+C, Ctrl+Shift+V paste through the Phase 69 `wrap_paste`, all over the Phase 105 `ClipboardStore` on `term`'s existing single `"display"` handle (`term` becomes its second real client after `clip-smoke`). Two cross-cutting fixes fell out: `MouseReporter::encode` now reports wheel notches to a mouse-tracking app as the xterm **64/65 pseudo-buttons** (it previously returned `None`, so `less`/`htop`/`tmux` saw no wheel at all), and `display_server` now delivers **surface-local** pointer coordinates (`PointerRouteDecision::deliver_origin`) stamped with **live keyboard modifiers** (`InputDispatcher::modifiers()`) — which also silently fixes `m3ui`/`settings`/`imgview` widget hit-testing and is what makes the Shift-drag override reachable at all. The gate also exposed a **Phase 29 PTY bug** that made the phase's own goal unreachable: `sys_write` on a full 4 KiB PTY ring returned `0` instead of `-EAGAIN`, which Rust's `std` reports as `WriteZero` and `println!` panics on — so the shell inside `term` was killed by SIGABRT whenever a program printed faster than `term` drained (running `dmesg` killed the terminal every time). Both PTY `sys_write` arms now return a partial count, `-EAGAIN`, `-EINTR`, or park on the existing (previously unused) wake queue, with host-tested readiness predicates in `kernel-core/src/pty.rs`. Feature work is ring-3; that PTY fix is the phase's only ring-0 change. Gate: `term-daily-driver-smoke` (`M3OS_TERM_POLISH_REGRESSION=1`), six QMP/PPM arms. PS/2 IntelliMouse wheel, selection re-anchoring, and an implicit pointer grab remain named deferrals. | Complete | `phase-112` | [Phase 112](./112-terminal-daily-driver-polish.md) | [Tasks](./tasks/112-terminal-daily-driver-polish-tasks.md) |

...
| 102 | I2C-HID Touchpad (Intel LPSS) | The real built-in pointer (the laptop has no PS/2 pointer): an Intel LPSS DesignWare I2C controller (`dwiic` ref) + I2C-HID transport + multitouch report parse (reusing the Phase 92b HID Report-Protocol decode) → `mouse_server` inject, replacing the Phase 100 interim USB mouse. Depends on 101. | In progress — Tracks A/B/C (host-tested pure-logic core: DesignWare transfer planner + `TX_ABRT` decode, HID-over-I2C v1.0 codec, `decode_touchpad_report`) landed + green; Track D (ring-3 `i2c-hid` daemon + wiring) + Track E (Dell HW) pend | `phase-102` | [Phase 102](./102-i2c-hid-touchpad.md) | [Tasks](./tasks/102-i2c-hid-touchpad-tasks.md) |

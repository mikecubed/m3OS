...
| 90a | Memory Protection Keys (PKU) + JIT V8 | Evolve the Phase 75 W^X invariant to a hardware-enforced per-thread **W^X v2** via x86 Memory Protection Keys (CR4.PKE, PTE key bits 59–62, `pkey_alloc`/`pkey_free`/`pkey_mprotect`, PKRU on the existing per-task XSAVE) — unguarded RWX stays rejected (`wx-violation` unchanged); a W+X mapping is permitted only under a non-default pkey whose default policy denies write. On that substrate, a **JIT-enabled `build_node` variant** (own content key; the 89 jitless artifact stays the default/fallback) runs V8 codegen + **working WASM** on m3OS (`pku-smoke` + `node-jit-smoke`), unblocking the 90b Claude Code TUI. Kernel `0.90.0`. | Planned | `phase-90a` | [Phase 90a](./90a-memory-protection-keys.md) | [Tasks](./tasks/90a-memory-protection-keys-tasks.md) |
| 90b | Claude Code | A modern CLI coding agent runs on the post-1.0 m3OS developer platform — **interactive TUI first** on the 90a JIT node variant (`yoga.wasm` needs WASM; jitless `claude -p` is the documented fallback/automation floor) — a pinned `@anthropic-ai/claude-code` npm tarball sealed as a `claude-code` `.m3pkg` (`DEPS=node`, offline solver install — Phase 89 proved live `npm install` impractical over the slow VFS), a `/usr/bin/claude` launcher pinning the supported env (86a CA bundle, no auto-update, no telemetry egress), `gh`-pattern 0600 credential seeding (**subscription** `claude setup-token` OAuth token primary; `ANTHROPIC_API_KEY` alternative; in-OS `/login` paste-flow as the human path), and a `claude-smoke` gate (always-on offline install+launch core; opt-in live API round-trip + file/shell/git agent workflow + QMP/PPM TUI render proof). Kernel `0.90.1`. | Planned | `phase-90b` | [Phase 90b](./90b-claude-code.md) | [Tasks](./tasks/90b-claude-code-tasks.md) |

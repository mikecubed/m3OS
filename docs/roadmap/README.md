...
| 97 | `dlopen-test-smoke` Intermittent TCG Stall (debugging) | Root-cause + fix the intermittent `smoke-test` **step 26** (`dlopen-test-smoke`) failure under plain TCG. The test demand-pages ~5 small `.so`s via the Phase 95b `MAP_LAZY_FILE` blocking page-fault→`vfs_server` read, and intermittently either stalls (no sentinel within the wait) or only emits `PASS` after the window. Ruled out: the trivial single-`write` destructor and the clean `dlclose` path; leading hypothesis is a **scheduler/futex lost-wakeup-class stall** in the blocking demand-read path (not a `dlopen` logic bug). Surfaced while landing the stat-identity fix (`ae01ed4`), which made steps 1–25 pass so the runner now reaches step 26. | **Open / deferred** — mitigation only (`dlopen-test-smoke` wait widened 30s→120s in `ae01ed4`); pre-existing (fails on `origin/main`); slow-VFS is largely a TCG artifact (passes under KVM) | `phase-97` | [Handoff](../handoffs/2026-06-26-dlopen-smoke-tcg-stall.md) | — |

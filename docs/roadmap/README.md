...
| 90 | Claude Code | A modern CLI coding agent runs on the post-1.0 m3OS developer platform — a pinned `@anthropic-ai/claude-code` npm tarball sealed as a `claude-code` `.m3pkg` (`DEPS=node`, offline solver install — Phase 89 proved live `npm install` impractical over the slow VFS), a `/usr/bin/claude` launcher pinning the supported env (86a CA bundle, no auto-update, no telemetry egress), `gh`-pattern 0600 `ANTHROPIC_API_KEY` seeding, and a `claude-smoke` gate (always-on offline install+launch core; opt-in live API round-trip + file/shell/git agent workflow). Kernel `0.90.0`. | Planned | `phase-90` | [Phase 90](./90-claude-code.md) | [Tasks](./tasks/90-claude-code-tasks.md) |

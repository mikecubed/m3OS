...
| Phase | Theme | Primary Outcome | Status | Source Ref | Milestone | Tasks |
|---|---|---|---|---|---|---|
| 69 | Terminal Contract Foundations | terminfo entry (`m3os-term`), alternate screen buffer, 256-color/truecolor SGR, SGR mouse reporting, DECSCUSR cursor styling, bracketed paste, SIGWINCH propagation. Hand-rolled `tui-smoke` byte-level validator. | Planned | `phase-69` | [Phase 69](./69-terminal-tui-capabilities.md) | [Tasks](./tasks/69-terminal-tui-capabilities-tasks.md) |
| 69a | Termios Raw Mode and Line Discipline | Full POSIX termios contract — `c_iflag`/`c_oflag`/`c_cflag`/`c_lflag`/`c_cc`, VMIN/VTIME, ISIG signal-from-terminal, `TCGETS`/`TCSETS` on TTY0 and PTY slave. Editors get byte-accurate raw-mode input. | Planned | `phase-69a` | [Phase 69a](./69a-terminal-termios.md) | [Tasks](./tasks/69a-terminal-termios-tasks.md) |
| 69b | UTF-8 Wire Decoding + Bitmap Glyph Expansion | UTF-8 decoder in `Screen::feed`; widen `ConsoleCmd::PutChar` to `u32`; bitmap font extended to cover Latin-1 supplement (U+0080–U+00FF) + Unicode box-drawing (U+2500–U+257F); EAW wide-cell accounting; `IUTF8` erase. | Planned | `phase-69b` | [Phase 69b](./69b-terminal-utf8-and-glyphs.md) | [Tasks](./tasks/69b-terminal-utf8-and-glyphs-tasks.md) |
| 69c | TTF Font Loader and Nerd Font Asset | TTF/OTF parser + glyph rasterizer + bounded LRU atlas; JetBrainsMono Nerd Font staged at `/usr/share/fonts/m3os/term.ttf`; runtime atlas-backed `Renderer::glyph_pixels` with static-table fallback. | Planned | `phase-69c` | [Phase 69c](./69c-terminal-font-infrastructure.md) | [Tasks](./tasks/69c-terminal-font-infrastructure-tasks.md) |
| 69d | ncurses Port and First Quality TUI Apps | `ports/lib/ncurses` (narrow + wide); `ports/util/less` + `htop` + `tmux` (with `libevent`); `cargo xtask tui-app-smoke` scripted validation. End-to-end proof of the 69-series terminal contract. | Planned | `phase-69d` | [Phase 69d](./69d-tui-app-foundation.md) | [Tasks](./tasks/69d-tui-app-foundation-tasks.md) |

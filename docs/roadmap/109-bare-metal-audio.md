...
**Depends on:** Phase 80 (Intel HDA Audio — out-of-process ring-3 `hda` driver behind `driver_ipc::audio`) ✅, Phase 63 (Audio PCM Emission / `audio_server` mixer) ✅, Phase 108 (HP OmniBook / AMD Strix Point — sequencing prerequisite: the Dell line proves the stack first), Phase 101 (ACPI — soft dependency for SoundWire/NHLT resource enumeration)

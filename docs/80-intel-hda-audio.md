...
`INTCTL` (GIE + per-stream IE) is armed and a handler decodes `INTSTS` + clears `SDnSTS.BCIS` (host-tested in `kernel_core::hda::irq`). The device-host IRQ allocator was extended to route **audio class `0x04` through INTx** (like the Phase 79 Ethernet fix) because the ring-3 HDA driver drives the legacy `INTCTL`/`SDnSTS` model with no MSI-X cause routing — a kernel-enabled MSI-X vector would stay silent. The IRQ subscription succeeds and INTCTL is armed; under QEMU's `intel-hda` the live BCIS notification is not observed reaching the ring-3 driver, so per the deferred-DPB decision the driver uses `SDnLPIB` polling as the authoritative completion path and proactively clears BCIS on the poll path so the armed level-triggered INTx can never storm.

# Dynamic Linker — Scaffolding (Phase 76)

**Aligned Roadmap Phase:** Phase 76
**Status:** Implemented (scaffolding only — 76b/76c/76d ship the rest)
**Source Ref:** phase-76
...
| `kernel/src/fs/ramdisk.rs` | Embeds the linker at `/lib/ld-musl-x86_64.so.1` |
| `userspace/ld-musl-x86_64.so.1/src/main.rs` | Transfer-only `_dlstart` stub |
| `userspace/dynlink_smoke/dynlink-smoke.c` | Smoke binary: `PT_INTERP` set, no `DT_NEEDED` |
| `userspace/smoke-runner/src/main.rs` | Drives `dynlink_smoke` and asserts the sentinel |
| `xtask/src/main.rs` | `build_ldso` + `build_dynlink_smoke` + `/lib/` staging |
...
## What is intentionally NOT in 76

| Concern | Subphase |
|---|---|
| `_dlstart` self-relocation (the linker is a PIE; its own `R_X86_64_RELATIVE` entries must be applied before any global access) | 76b |
| `PT_DYNAMIC` parsing, `DT_NEEDED` resolution, dependency graph, topological sort | 76b |
| x86_64 relocations applied to loaded `.so` files: `R_X86_64_GLOB_DAT`, `R_X86_64_JUMP_SLOT`, `R_X86_64_RELATIVE`, `R_X86_64_64` | 76b |
| `DT_INIT` / `DT_INIT_ARRAY` constructor running | 76b |

...
| [Native Rust Toolchain (on-device `rustc`)](./95-native-rust-toolchain.md) | 95 | m3OS self-hosting for Rust — a fully-static `x86_64-unknown-linux-musl` `rustc` (host-cross-built from Rust 1.96.0 via upstream `x.py`) packaged behind `M3OS_WITH_RUST` that compiles + links (bundled `rust-lld`) + runs a Rust source file **on-device** (`RUSTC_OK`) against a prebuilt std sysroot resolved relative to the binary; covers the *running a program* vs *running the compiler* distinction (the Rust analog of Phase 85d Clang, not Phase 86d Go), the static musl-host bootstrap (host clang as the musl cross, `download-ci-llvm`-driven musl LLVM, the reused Phase 85d libc++ sysroot), the sysroot relocation contract, the reused `rust-lld` on a `cc`-less m3OS, the reused streaming-exec/`pread64` kernel substrate, and the proc-macro `dlopen`/`libc.so` (Phase 93) wall. Links the [95 design](./roadmap/95-native-rust-toolchain.md) + [task](./roadmap/tasks/95-native-rust-toolchain-tasks.md) docs |

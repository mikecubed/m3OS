...
- `cargo xtask ahci-root-smoke` makes that root-over-SATA path a **CI gate** (previously manual-only): it routes the real ext2 data disk to `ich9-ahci` and asserts, in order, `[m3os] Hello from kernel` → `init: / mount failed (` (virtio root absent) → `AHCI: ext2 partition found start=` (read-only MBR probe on the real disk, not the destructive self-test) → `[blk::remote] auto-registered ring-3 'ahci0' driver` (the owner trust gate executes and accepts) → `init: / mounted (ext2 via ring-3 ahci.block)` (load-bearing) → `m3OS login:` (init scanned and read `/etc/services.d/*` — which live on the ext2 data disk, not the ramdisk — back over `ahci.block` and finished service boot). The mount marker is the unforgeable proof; the login prompt corroborates a full, healthy boot. Passes 6/6 steps in ~8s on QEMU TCG. This is the only executing coverage of the `blk::remote` owner gate and the full `RemoteBlockDevice` AHCI data path.

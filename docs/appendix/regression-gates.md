...
## meltdown-poc-smoke + rop-cet-poc-smoke

**Env var:** `M3OS_SEC_POC_REGRESSION=1`

Phase 110 Dell-validation PoCs — the userspace binaries that *functionally* prove the two Track A/B defenses on real silicon (`/bin/meltdown-poc`, A.6; `/bin/rop-cet-poc`, B.3). These CI gates are **run-to-completion checks only**: QEMU TCG models no caches, no speculation, and no CET, so the actual leak-reject and `#CP`-kill security assertions are **bare-metal-only** and live in the [2026-07-09 Dell validation runbook](../handoffs/2026-07-09-dell-validation-session.md) (Blocks 2a/2b). What they assert under QEMU:
...
Set on branches touching `userspace/meltdown-poc`, `userspace/rop-cet-poc`, the CET `#CP` handler (`control_protection_fault_body`), or the KPTI/PCID substrate.

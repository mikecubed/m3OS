...
| 57a | Scheduler Block/Wake Protocol Rewrite | **Planned** | AGENTS.md and 57b/c list 57a as `✅`. Git log shows 57e fixes (Bug #12, Bug #13) on `feat/audit-status` branch presupposing 57a–57d done. **Code has 4 `TODO(57a-C/D): route through pi_lock + with_block_state` markers in scheduler.rs at sites that bypass the abstraction** | 🛑 | Status drift; pi_lock wiring missing at 4 callsites |
| 57b | Preemption Foundation | **Complete pending soak** | PR #132 referenced; **no soak result document** in any accessible doc. `preempt_count == 0` debug assertion is the dominant risk. Kernel version bump to 0.57.2 unverified | 🟡 | Soak status unknown |
| 57c | Kernel Busy-Wait Conversion | Complete | Independent of 57b; converts unbounded spins; annotates hardware-bounded spins. **`preempt_disable` wrappers at annotated sites are deferred to 57e Track B** — currently comments only | ✅ | Honest split |
| 57d | Voluntary Preemption | **Planned** (functionally complete per dep list) | Two distinct trap-frame types finalised (`PreemptTrapFrameUser` + `PreemptTrapFrameKernel`). Ring-0 path stays non-preemptible | 🟡 | — |
| 57e | Full Kernel Preemption | **Planned** (active work on this branch) | Recent commits fix Bug #12 (kernel-mode preempt quantum / `preempt_enable` deferral, multi-part) and Bug #13 (`wake_child_waiters` wake-side bracketing). XSAVE migration (Track J) load-bearing for hosted AVX binaries | 🟠 | Bugs not surfaced in design doc |
...
3. **Status-field drift in the live phases** — 55a, 55b, 56, 57a all say "Planned" in their design doc headers while every downstream phase and AGENTS.md treats them as completed dependencies. This makes the README's status column unreliable as a source of truth.

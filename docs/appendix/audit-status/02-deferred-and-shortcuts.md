...
- **5 tick-multiplier bugs latent.** Phase 57a doc lists 5 sites with `× 10` / `÷ 10` factor assuming `TICKS_PER_SEC = 100` when actual rate is 1000 — `scheduler.rs:1892`, `scheduler.rs:2191`, `syscall/mod.rs:14647` (sys_poll), `syscall/mod.rs:14894` (select_inner), `syscall/mod.rs:15304` (sys_epoll_wait).

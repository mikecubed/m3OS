...
| 0x1100–0x1115 | ipc_* | ipc | Mechanism | IPC operations dispatched through `kernel/src/ipc/mod.rs`; maps userspace `0x1100`–`0x1115` to internal dispatch 1–22 (includes recv/send/call/reply, cap_grant, notifications, service register/lookup, bulk reply drains, non-blocking recv, cap-free service-existence queries, and the scheduler-blocking `ipc_wait_service` readiness primitive) |

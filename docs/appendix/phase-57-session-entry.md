...
| 5 | `greeter` | **Phase 71** — greeter's service registry entry (`"greeter"`) becomes available. Greeter renders the GUI login form and reads username + password from the `kbd_server` event stream | Per-service retry up to **3** attempts; on exhaustion, escalate to `text-fallback` |
| 6 | `term` | `term`'s service registry entry (`"term"`) becomes available AND the AF_UNIX listening socket at `/run/m3os/term.sock` (or whatever G.6 settles on) is bindable. **Phase 71** — term is no longer auto-started by init; it registers only after greeter successfully authenticates a user and `setuid`s + `execve`s `/bin/term` in-process | Per-service retry up to **3** attempts; on exhaustion, escalate to `text-fallback` |
...
- **Greeter before term (Phase 71).** The graphical login manager is the gate between the boot-time service set and a user session. `greeter` runs as root, reads `/etc/passwd` + `/etc/shadow`, drops credentials via `setgid` + `setuid`, then `execve`s `/bin/term` in-process so term inherits the authenticated UID/GID. Until Phase 71 this step did not exist and term ran as root by default.
- **`term` last.** `term` is a regular client of every service above it. Starting it last keeps the dependency graph explicit and matches the Phase 56 manifest convention (`gfx-demo.conf` declares `depends=display`). After Phase 71 term's lifetime is rooted in greeter rather than init — see the greeter → term hand-off described in the row above.

...
|---|---|---|---|
| `virtio_blk::do_request` | `kernel/src/blk/virtio_blk.rs` | Completion ring IRQ → `REQ_WOKEN` flag | B.1: verify, add regression test |
| `sys_poll` no-waiter loop | `kernel/src/arch/x86_64/syscall/mod.rs` | `WaitQueue::sleep` | B.1: verify |
| `net_task` NIC wake | `kernel/src/main.rs` | `block_current_until(TaskState::BlockedOnRecv, &NIC_WOKEN, None)` | B.2: verify |

...
| `kernel/src/rtc.rs:90` | `read_rtc` (update-in-progress wait) | `while update_in_progress() && spins < MAX_UIP_SPINS` | RTC chip (MC146818) hardware clears UIP bit when its internal update cycle completes | HW-bounded: MC146818 update window is exactly 248 µs max (Motorola MC146818A datasheet §2.3); MAX_UIP_SPINS provides a hard upper limit | **annotate** (C.5) | RTC hardware update window; already has a spin-count guard. Not on a user-visible hot path. |

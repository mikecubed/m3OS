...
| `power-smoke` | `M3OS_POWER_REGRESSION=1` | powerd serves the VM no-battery/no-zones/no-HWP posture through acpid, the ring-3 conservative governor ticks against the `0x116x` cpufreq syscalls, `m3ctl power status` renders battery+thermal+governor over the `power` IPC service, and a QMP power button drives the full D.3 poweroff chain (powerd → `/bin/shutdown` → init teardown → kernel sync → ACPI S5) to a guest-initiated QEMU exit. Live battery/brightness/HWP-MSR arms are hardware-only. |

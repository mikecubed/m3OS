...
| `ptrace-gdbserver-smoke` | `M3OS_PTRACE_REGRESSION=1` | Phase 111 Track D.3 native gdbserver: boots the `ptrace`-featured kernel, logs in, and launches `m3gdbserver 1234 /bin/ptrace-tracee`. A **host GDB-RSP client over TCP** (SLIRP hostfwd) attaches and drives a full session — `?` → the tracee's `execve` exec-stop, `Z0` at the entry + `c` → the breakpoint hit (RIP=entry+1), `z0`+`P` rewind, `M`/`m` memory round-trip, `c` → run-to-exit `W07` — all translated through `m3gdbserver` → `sys_ptrace`. Proves the RSP↔ptrace path a real `gdb target remote` would take. |

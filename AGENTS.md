...
| `meltdown-poc-smoke` + `rop-cet-poc-smoke` | `M3OS_SEC_POC_REGRESSION=1` | QEMU **run-to-completion** checks for the Phase 110 Dell-validation PoCs (the leak-reject and `#CP`-kill *security* arms are bare-metal-only — QEMU TCG models no caches/speculation/CET). `meltdown-poc` loads (incl. its ~1 MiB flush+reload channel BSS) and runs the channel + calibration control + mispredicted-branch speculative kernel read without faulting (the read never architecturally retires → `MELTDOWN_POC:leak off=0`). `rop-cet-poc`'s naked return-address overwrite transfers to `pwned()` under no-CET (`ROP_CET_POC:PWNED`, never `after-NOT-OVERWRITTEN`). Full A.6/B.3 validation: `docs/handoffs/2026-07-09-dell-validation-session.md`. |

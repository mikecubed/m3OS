...
| `term-daily-driver-smoke` | `M3OS_TERM_POLISH_REGRESSION=1` | `term` scrollback viewport + selection/clipboard on the QMP/PPM dump: Shift+PageUp brings evicted rows back and a keystroke snaps to the live tail; a Report-protocol `usb-tablet` wheel scrolls the same viewport; a mouse drag paints a highlight and copy-on-release lands in the compositor `ClipboardStore` (read back by an independent `clip-smoke --paste` client); Ctrl+Shift+V pastes bracketed. |

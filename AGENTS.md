...
| `power-smoke` | `M3OS_POWER_REGRESSION=1` | powerd serves the VM no-battery posture through acpid's `ACPI_EVAL`, `m3ctl power status` renders it over the `power` IPC service, and a QMP power button reaches powerd's event subscription. Live battery/brightness/thermal arms are hardware-only. |

...
**m3OS** (technical name: `m3os`) is a bootable microkernel OS in Rust: x86_64, UEFI boot, kernel **v0.85.3**. Ring 0 handles memory, scheduling, IPC/capabilities, interrupt routing, and in-kernel drivers; ring 3 hosts everything else.
...
| `clang-smoke` (Phase 85d: host-cross-builds an opt-in, X86-only, statically musl-linked **clang + lld** (+ a self-contained libc++) into a `.m3pkg`, builds a fresh image with the `M3OS_WITH_CLANG` feature so it bundles into `/usr/pkg/`, boots m3OS, `pkg install clang` (no deps), then INSIDE m3OS: `clang --version` reports 18.1.8, `clang -print-resource-dir` resolves under `/usr` (relocation contract), `clang -O2 /usr/src/hello.c` compiles + links (lld) + **runs** (`CLANG_C_OK`), `clang++ /usr/src/hello.cpp` links the bundled libc++ + runs (`CLANG_CPP_OK`), and `clang -fuse-ld=lld` links via LLD. Runs at `--timeout 2400` because the several-hundred-MB install and each cold clang invocation over the slow ring-3 VFS take minutes. Clang is static + X86-only; the host clang is the cross-compiler since musl-tools has no C++ compiler. Requires host clang + cmake + ninja for the initial build) | `M3OS_CLANG_REGRESSION=1` |

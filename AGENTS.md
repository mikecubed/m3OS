...
| `node-smoke` (Phase 89: cross-builds the fully-static jitless Node.js 22 + npm `.m3pkg` via build_node, builds a fresh `M3OS_WITH_NODE` image so it bundles into `/usr/pkg/`, boots m3OS, `pkg install node`, then over serial: `node --version` is 22.x; `node /usr/src/node-probe.js` emits NODE_HELLO_OK/FS/PROC/EVENTLOOP/TIMER (the A.1 timerfd event-loop path); `node /usr/src/node-http.js` does a loopback `http.createServer`+`http.get` (NODE_HTTP_OK); `node /usr/src/node-http-egress.js` does a plaintext HTTP GET over the in-kernel TCP stack to a host server via SLIRP guestfwd (NODE_EGRESS_OK, always-on). The TLS/DNS/`npm install` arms are opt-in `M3OS_NODE_NET=1` (live HTTPS cert-validate + `npm install is-number` over real egress) and skip-with-reason otherwise. Runs at `--timeout 5400` (the ~100 MB install + cold static-binary load over the slow VFS). Absent a host C++ toolchain (clang/lld/python3/make) + the llvm musl sysroot, the gate prints SKIP and returns success.) | `M3OS_NODE_REGRESSION=1` |

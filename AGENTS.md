...
| `screenshot-smoke` | `M3OS_SCREENSHOT_REGRESSION=1` | `screenshot` captures the composited frame via `CaptureOutput` into a client SHM, PNG-encodes it, and re-decodes the written file losslessly. |
...
| `term-daily-driver-smoke` | `M3OS_TERM_POLISH_REGRESSION=1` | Six QMP/PPM arms over `term`'s scrollback viewport, selection/clipboard, and the wheel's two destinations: Shift+PageUp brings evicted rows back and a keystroke snaps to the live tail; a Report-protocol `usb-tablet` wheel scrolls that same viewport; a mouse drag paints a highlight and copy-on-release lands in the compositor `ClipboardStore` (read back line-exact by an independent `clip-smoke --paste` client); Ctrl+Shift+V delivers the offer to the PTY with `term` still on screen; the identical wheel notch instead reaches a mouse-tracking app on the alternate screen (`tui-smoke mouse-live` — serial `cb=64`/`cb=65` plus a per-notch hue+region check); and a Shift-held drag force-selects where an unshifted one does not, proving the compositor's live-modifier stamping. The `ESC[200~` framing is **not** asserted (nothing in the image enables `?2004h`; covered by the `wrap_paste` host tests). |
| `graphical-boot-smoke` | `M3OS_GRAPHICAL_BOOT_REGRESSION=1` | The **only** harness that boots the `graphical_login = true` lane `cargo xtask run-gui` takes; every other gate boots `graphical_login = false`, which stages three fewer service confs. Asserts init *loads* `wallpaper`/`bar`/`notifyd` (the `MAX_SERVICES` overflow guard — init drops any conf past the ceiling with no diagnostic, which is how the top bar went missing for two phases while every gate stayed green), that each starts, that `bar` defers to the greeter rather than piercing the login screen, that a root/root GUI login completes and the `/run/m3os-current-session` handshake reaches `bar` (which polls for it with no timeout), that `bar`'s Layer surface actually composites over the top 48 px with real widgets (band is not compositor background + ≥4 distinct colours, on the QMP/PPM dump), and that `SUPER+RETURN` still spawns a terminal — the graphical lane ships no `[autostart]`, so that chord is the only route to a shell after a GUI login. |
...
| `settings-smoke` | `M3OS_SETTINGS_REGRESSION=1` | `settings` panel volume slider drives audio `SetMasterVolume` end to end (keyboard → widget → IPC → server gain state) with a visible repaint on the QMP/PPM dump; the Display/Power sections connect to the Phase 103 `power` service and render the QEMU posture (`SETTINGS:power=ok battery=none backlight=none sleep=S3+S4`). |
| `symphonia-smoke` | `M3OS_SYMPHONIA_REGRESSION=1` | `symphonia-play` decodes the WAV + FLAC fixtures and plays them audibly through `audio_server` (captured WAV non-silent). |
...
| `mitigations-status-smoke` | `M3OS_MITIGATIONS_REGRESSION=1` | `m3ctl mitigations status` reports correct Spectre-v2/retpoline/KPTI posture at boot — incl. live KPTI enforcement on the default `auto` lane (`kpti(policy=true active=true)` boot line + reporter `Meltdown: Mitigation: PTI`, Phase 110 A.4) and the Phase 110 A.5 PCID posture (`pcid(active=false supported=false)` boot line + `KPTI PCID: fallback` reporter line — QEMU TCG has no PCID/INVPCID; the PCID-active arm is bare-metal-only) and the Phase 110 B.3 CET posture (`cet(active=false supported=false)` boot line + `CET: not-supported` reporter line — QEMU TCG models no CET; the CET-active arm is bare-metal-only). |
...
| `meltdown-poc-smoke` + `rop-cet-poc-smoke` + `nested-sig-cet-poc-smoke` | `M3OS_SEC_POC_REGRESSION=1` | QEMU **run-to-completion** checks for the Phase 110 Dell-validation PoCs (the leak-reject and `#CP`-kill *security* arms are bare-metal-only — QEMU TCG models no caches/speculation/CET). `meltdown-poc` loads (incl. its ~1 MiB flush+reload channel BSS) and runs the channel + calibration control + mispredicted-branch speculative kernel read without faulting (the read never architecturally retires → `MELTDOWN_POC:leak off=0`). `rop-cet-poc`'s naked return-address overwrite transfers to `pwned()` under no-CET (`ROP_CET_POC:PWNED`, never `after-NOT-OVERWRITTEN`). `nested-sig-cet-poc` proves an outer signal handler that self-raises a nested signal has both handlers nest and return (`NESTED_SIG_POC:PASS`) — the CET-independent half of the nested-signal shadow-stack fix (live-SSP seed/`sigreturn` re-sync; the nested-`#CP`-kill arm is bare-metal-only). Full A.6/B.3 validation: `docs/handoffs/2026-07-09-dell-validation-session.md`. |
| `kpti-selftest-smoke` | `M3OS_KPTI_REGRESSION=1` | Phase 110 Track A.1–A.3b KPTI substrate: the stock-kernel boot self-test (`mm::kpti`) builds a real user PML4 via the production `build_user_half` (synthetic user page + full entry set — every online core's PerCoreData/GDT/TSS + NMI/`#DF` IST tops + KPTI trampoline-stack top page, the page-aligned `.text.kpti_entry` + `.text.kpti_irq_entry` + `.text.kpti_exit` sections (layout-asserted), and the IDT, all via private sub-tables at their kernel VAs; the interrupt-delivery structures are `PageIsolated` and their page alignment asserted), walks it to prove via `kernel_core::kpti::check_user_half_invariant` that no kernel image / heap / kstack / direct-map leaf is reachable from the user CR3, round-trips every entry-set page to prove it's reachable, asserts the task-kstack top page is NOT reachable, and exercises the `#PF`-time top-level-slot sync (stale a user slot → `sync_slot_raw` repairs it → second sync is a no-op) (`KPTI_SELFTEST:PASS`) — the Meltdown-isolation invariant QEMU cannot exercise directly. **A.4 activated KPTI** (`KPTI_WIRED=true`): the user half shares both user slots (`USER_PML4_SLOTS=[0,255]` — image/mmap + stack), the per-core CR3 pair is published at every dispatch locus, and the default QEMU `auto` boot (TCG `rdcl_no=false`) runs ring 3 on the user half live — so this gate (and every other) also exercises the live trampoline end to end. **Post-A.5 hardening (18–21/n)**: TSS.RSP0 targets a per-CPU trampoline stack while KPTI is active — entry stubs copy ring-3 frames to the task kstack after the CR3 switch, exits build their `iretq` frames on it — so no kernel-stack page is user-mapped (the A.3b ~4 KiB kstack self-leak is closed) and `CLONE_VM` threads need no per-thread map. |
| `debug-substrate-smoke` | `M3OS_DEBUG_SUBSTRATE_REGRESSION=1` | Phase 111 Track B trap/debug-register substrate: boot self-test proves the `#BP` RIP-fixup, the `RFLAGS.TF` single-step (exactly one `#DB`), and `DR7` arm/disarm (DR6/DR7 codec is an always-on `kernel-core` host test). |
| `kgdb-smoke` | `M3OS_KGDB_REGRESSION=1` | Phase 111 Track C in-kernel GDB stub: boots with the `kgdb` feature frozen at a wait-for-debugger `int3`, a raw GDB-RSP client attaches over COM2→TCP, sets a `Z0` breakpoint at `kgdb_probe_target` (`nm` vaddr + the 1 TiB PIE base), continues, and the hit is asserted with `RIP` at the breakpoint + a `KGDB_PROBE_MAGIC` `m` read-back; then it runs the guest free and sends a bare `0x03` (Ctrl-C) to assert an **async break** into the running kernel (the BSP timer tick polls COM2). The SMP all-stop sentinel confirms no sibling core advanced while stopped. Stub is OFF in production (arbitrary kernel peek/poke). |
| `ptrace-smoke` | `M3OS_PTRACE_REGRESSION=1` | Phase 111 Track D ptrace userspace debugging: builds the kernel with the `ptrace` feature (so `sys_ptrace` exists and the #BP gate is DPL 3) and runs `/bin/ptrace-test` — a forked tracee calls `PTRACE_TRACEME` + `int3` and stops-and-notifies its tracer, which asserts the stop (`WIFSTOPPED`/`SIGTRAP`), `GETREGS`/`SETREGS`, `PEEKTEXT`/`POKETEXT` (round-trip through the tracee's page tables), and `CONT` — the `SETREGS`'d register is observed in the tracee's exit code. Feature OFF in production (arbitrary cross-process peek/poke). |
| `ptrace-gdbserver-smoke` | `M3OS_PTRACE_REGRESSION=1` | Phase 111 Track D.3 native gdbserver: boots the `ptrace`-featured kernel, logs in, and launches `m3gdbserver 1234 /bin/ptrace-tracee`. A **host GDB-RSP client over TCP** (SLIRP hostfwd) attaches and drives a full session — `?` → the tracee's `execve` exec-stop, `s` → single-step (RIP advances one instruction, exercising the ring-3 `#DB`/`on_user_debug` consumer), `Z0` + `c` → the breakpoint hit (RIP=stepped+1), `z0`+`P` rewind, `M`/`m` memory round-trip, `c` → run-to-exit `W07` — all translated through `m3gdbserver` → `sys_ptrace`. Proves the RSP↔ptrace path a real `gdb target remote` would take. |
...
| `power-smoke` | `M3OS_POWER_REGRESSION=1` | powerd serves the VM no-battery/no-zones/no-HWP/no-panel posture through acpid (with live q35 `\_S3`/`\_S4` sleep discovery), the ring-3 conservative governor ticks against the `0x116x` cpufreq syscalls, `m3ctl power status` renders battery+thermal+governor+backlight over the `power` IPC service (brightness set rejected cleanly with no panel; `power suspend` fails closed to a live session on this S3-disabled lane), and a QMP power button drives the full D.3 poweroff chain (powerd → `/bin/shutdown` → init teardown → kernel sync → ACPI S5) to a guest-initiated QEMU exit. Live battery/brightness/HWP-MSR arms are hardware-only. |
| `suspend-smoke` | `M3OS_SUSPEND_REGRESSION=1` | Full ACPI S3 suspend/resume round trip: kernel quiesce (AP park, virtio drain, PCI save) → FACS X-vector wake shim → kernel re-init → `\_WAK` → live shell + disk post-resume, and the power button still drives the poweroff chain to a guest-initiated QEMU exit. `power-smoke` is the S3-disabled fail-closed lane. |
| `usb-root-smoke` | `M3OS_USB_ROOT_REGRESSION=1` | The combined GPT USB image (`image --combined`) boots as the machine's only (USB) disk: OVMF loads the ESP, init brings up xhci + usb-storage, the root slot adopts `usb0.block`, the GPT scan finds the ext2 partition, and a serial login writes + reads back a file on the writable USB root. |
| `nvme-rw-smoke` + `nvme-persist-smoke` | `M3OS_NVME_REGRESSION=1` | NVMe root suite (analogs of the always-on AHCI gates): the real ext2 rootfs routed behind a QEMU `nvme` controller mounts over `nvme.block` (init's Stage-1 NVMe arm), a 200 KiB ext2 write + fresh-process read-back round-trips the ring-3 `nvme_driver` write path, and a two-boot marker survives a reboot + fresh remount (BLK_FLUSH, no flush-failure log). |
| `nvme-install-smoke` + `nvme-install-part-smoke` | `M3OS_NVME_INSTALL_REGRESSION=1` | Two-boot end-to-end installer proofs: the combined USB image boots with a blank NVMe attached, `/sbin/installer` installs USB→NVMe over the `0x117x` syscalls (raw arm: sparse `dd`-copy; `--part` arm: fresh target-sized GPT + ESP copy + on-device `mkfs.ext2` + file-level populate + first-user account prompts replacing the image's seeded credentials, host-cross-checked with the `gpt` crate + `e2fsck`), then the NVMe boots alone to a live login over `nvme.block` — as the created user on the part arm. |

...
| `kpti-selftest-smoke` | `M3OS_KPTI_REGRESSION=1` | Phase 110 Track A.1 KPTI user-half builder: the stock-kernel boot self-test (`mm::kpti`) builds a real user PML4 (synthetic user page + minimal entry set — PerCoreData, `syscall_entry` text, entry stack, mapped via private sub-tables at their kernel VAs), walks it, and proves via `kernel_core::kpti::check_user_half_invariant` that no kernel image / heap / kstack / direct-map leaf is reachable from the user CR3 (`KPTI_SELFTEST:PASS`) — the Meltdown-isolation invariant QEMU cannot exercise directly. Builder only; `KPTI_WIRED` is still off (the live CR3 trampoline is A.2–A.4). |

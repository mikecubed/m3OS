...
| `nvme-install-smoke` | `M3OS_NVME_INSTALL_REGRESSION=1` | Two-boot end-to-end installer proof: the combined USB image boots with a blank NVMe attached, `/sbin/installer` sparse-raw-copies USB→NVMe over the `0x117x` syscalls (usb-storage shm bulk path) + flush + reboot, then the NVMe boots alone to a live login over `nvme.block`. |

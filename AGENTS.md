...
| `mitigations-status-smoke` | `M3OS_MITIGATIONS_REGRESSION=1` | `m3ctl mitigations status` reports correct Spectre-v2/retpoline/KPTI posture at boot — incl. live KPTI enforcement on the default `auto` lane (`kpti(policy=true active=true)` boot line + reporter `Meltdown: Mitigation: PTI`, Phase 110 A.4). |

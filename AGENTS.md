...
| `kpti-selftest-smoke` | `M3OS_KPTI_REGRESSION=1` | Phase 110 Track A.1+A.2 KPTI substrate: the stock-kernel boot self-test (`mm::kpti`) builds a real user PML4 (synthetic user page + minimal entry set — PerCoreData, the page-aligned `.text.kpti_entry` section (both SYSCALL stubs + shared body, layout-asserted), entry stack, mapped via private sub-tables at their kernel VAs), walks it, and proves via `kernel_core::kpti::check_user_half_invariant` that no kernel image / heap / kstack / direct-map leaf is reachable from the user CR3 (`KPTI_SELFTEST:PASS`) — the Meltdown-isolation invariant QEMU cannot exercise directly. The A.2 `syscall_entry_kpti` CR3 trampoline is LSTAR-selected on `kpti_active`, so production installs the non-KPTI stub; `KPTI_WIRED` is still off (IRQ/IST symmetry + activation are A.3–A.4). |

...
// Phase 110 Track B.3 — the signal-delivery/`sigreturn` shadow-stack SSP is no
// longer stashed in a per-task slot. Delivery seeds the restorer with `WRUSS`
// relative to the *live* SSP and `sigreturn` re-syncs `IA32_PL3_SSP` from the
// live SSP (`cet::seed_signal_shadow_stack` / `cet::restore_signal_ssp`), which
// is nesting-safe — see the fix in `arch/x86_64/cet.rs`. The old
// `save_current_task_signal_ssp` / `restore_current_task_signal_ssp` pair (and
// the `Task::cet_signal_ssp` field) were removed with this change.

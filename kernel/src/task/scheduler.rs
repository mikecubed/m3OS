...
pub fn preempt_enable() {
    ...
    // retry), the zero-crossing branch records `preempt_resched_pending
    ...
    //
    // Phase 57e Bug #12 retry: the original preempt-full branch yielded
    ...
    // STDIN_WAITQUEUE block); with the busy-yield removed, this retry
    // can defer the reschedule safely — the previously-load-bearing
    // forward-progress dependency on the eager-yield was specifically
    // the stdin busy-loop that no longer exists.  The Bug #6 brackets
    // around `deliver_message + wake_task_v2` remain load-bearing:
    // within the bracket `preempt_count > 0`, so no zero-cross fires
    // (eager or deferred), preserving the H7 lost-wake protection.
    let location = core::panic::Location::caller();

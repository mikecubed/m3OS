...
pub fn init_ap_per_core(core_id: u8, apic_id: u8) -> *mut PerCoreData {
    ...
    let kernel_stack_top = (kernel_stack.as_ptr() as u64) + SYSCALL_STACK_SIZE as u64;
...
pub unsafe fn per_core_gdt_init(data: &PerCoreData) {
    let tss: &'static TaskStateSegment = &*data.tss_ptr;

    // Allocate GDT on the heap and leak it.
    let gdt = Box::leak(Box::new(GlobalDescriptorTable::new()));

    // Layout must match BSP's GDT exactly:
    //   0x00  null          (implicit)

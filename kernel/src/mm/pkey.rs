...
//! |---|---|---|---|
//! | Demand fault commit (anonymous lazy) | `arch/x86_64/interrupts.rs::demand_map_user_page_locked` | scratch (`prot`) | **Routed through [`compose_user_pte_flags`]** — takes a `pkey` arg (0 until a VMA carries a key in Track B.3). Without the route a faulted-in JIT page would lose its tag. Eager paths are enumerated below. |
//! | Demand fault commit (lazy file-backed, Phase 95b) | `arch/x86_64/interrupts.rs::demand_map_user_page_from_buf_locked` | scratch (`prot`) + file bytes | **Routed through [`compose_user_pte_flags`]** — the `MAP_LAZY_FILE` loader path's per-page fill. Same key handling as the anonymous demand-fault row (the VMA's `pkey` is carried, 0 today), so a future pkey-tagged file mapping keeps its tag; the page content is read from the backing file instead of zero-filled. |
//! | File-backed mmap (eager, `MAP_SHARED` writeback only) | `arch/x86_64/syscall/mod.rs::sys_mmap_file_backed` (eager branch) | scratch (`prot`) | **Key 0 today** — composes `PRESENT \| USER_ACCESSIBLE` (+ `WRITABLE` / `NO_EXECUTE` from `prot`) without a key stamp. As of Phase 95b this branch serves only `MAP_SHARED` writeback mmaps; the `MAP_PRIVATE` loader path is lazy and composes via the demand-fault row above. Correct while no VMA carries a non-zero key. **B.3/C.1 must revisit** when VMAs gain keys: eager file-backed pages need the same `compose_user_pte_flags` route. |

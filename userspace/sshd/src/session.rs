...
async fn io_task(
    ...
    loop {
        ...
        if h9_io_iter <= 30 || h9_io_iter.is_multiple_of(50) {
            ...
            // can see whether output is getting stuck non-empty
            // post-hostkeys.
            let out_len = {
                let mut guard = runner.lock().await;
                guard.output_buf().len()
            };
            write_str(STDOUT_FILENO, "[h9-io] iter=");
            write_u64(STDOUT_FILENO, h9_io_iter);
            write_str(STDOUT_FILENO, " out_buf_len=");
            write_u64(STDOUT_FILENO, out_len as u64);
            write_str(STDOUT_FILENO, " pending_len=");
        ...
        //
        // H6 experiment: only arm output_waker when output_buf is drained. If
...
async fn progress_task(
    ...
    let mut h9_iter_count = 0u64;
    loop {
        h9_iter_count = h9_iter_count.saturating_add(1);
        if h9_iter_count <= 5 || h9_iter_count.is_multiple_of(50) {
            // H9: per-Notify counts so we can attribute the residual ping-pong
            // to a specific signaling primitive in the session-child executor.
            write_str(STDOUT_FILENO, "[h9-pn] iter=");
            write_u64(STDOUT_FILENO, h9_iter_count);
            write_str(STDOUT_FILENO, " progress_fired=");
            write_u64(STDOUT_FILENO, progress_notify.debug_fired());
            write_str(STDOUT_FILENO, " progress_pending=");
            write_u64(STDOUT_FILENO, progress_notify.debug_pending());
            write_str(STDOUT_FILENO, " session_fired=");
            write_u64(STDOUT_FILENO, session_notify.debug_fired());
            write_str(STDOUT_FILENO, " session_pending=");
            write_u64(STDOUT_FILENO, session_notify.debug_pending());
            write_str(STDOUT_FILENO, "\n");
        }
        ...
        match action {
            ...
            ProgressAction::ContinueProbe => {
                ...
                // re-acquire, so the predicates reflect the post-handler
                // state.
                let (input_ready, out_empty) = {
                    let mut g = runner.lock().await;
                    (g.is_input_ready(), g.output_buf().is_empty())
                };
                write_str(STDOUT_FILENO, "[h9-postkey] input_ready=");
                write_str(STDOUT_FILENO, if input_ready { "1" } else { "0" });
                write_str(STDOUT_FILENO, " out_empty=");
                write_str(STDOUT_FILENO, if out_empty { "1" } else { "0" });
                write_str(STDOUT_FILENO, "\n");
                yield_now().await;
            }
            ProgressAction::LoopContinue => {

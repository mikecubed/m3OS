...
//
// DESCRIPTION:  not so system specific sound interface — m3OS overlay.
//
// Phase 63a Track F: this overlay replaces upstream i_sound.c with
// an identical body except for the two registration sites:
//   - sound_modules[] now points at m3os_sound_module
//   - InitMusicModule() now installs &m3os_music_module
//
// Every other function in upstream is preserved verbatim. The patch
// only changes the registration list per the design-doc DRY rule.
//
...
// m3OS sound and music modules (Phase 63a).
...
// Compiled-in sound modules — Phase 63a Track F: register m3OS
// modules instead of (or alongside) upstream's DG_sound_module.
...
// Phase 63a Track F: install the m3OS music module directly.

static void InitMusicModule(void)
{
    music_module = &m3os_music_module;
}

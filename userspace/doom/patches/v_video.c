...
/* Shared debug fd — also opened by w_wad.c patch */
static int vv_dbg_fd = -1;

static void vv_dbg(const char *msg)
{
    if (vv_dbg_fd < 0)
        vv_dbg_fd = open("/tmp/doom_dbg.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
    if (vv_dbg_fd >= 0) { write(vv_dbg_fd, msg, strlen(msg)); fsync(vv_dbg_fd); }
}

#define VV_FMT(fmt, ...) \
    do { char _b[256]; snprintf(_b, sizeof(_b), fmt, ##__VA_ARGS__); vv_dbg(_b); } while(0)
...
void V_DrawPatch(int x, int y, patch_t *patch)
{
    if (patch == NULL)
    {
        VV_FMT("V_DrawPatch: NULL patch at x=%d y=%d caller=%p\n",
               x, y, __builtin_return_address(0));
        return;
    }

//! Minimal runtime for the `display_client_ffi` staticlib.
//!
//! Mirrors `audio_client_ffi::staticlib_runtime` exactly so that, when
//! DOOM links both `libaudio_client_ffi.a` and `libdisplay_client_ffi.a`
//! into the same musl-static binary, the resolver picks one set of
//! `#[panic_handler]` + `#[global_allocator]` symbols and the other is
//! discarded via `-Wl,--allow-multiple-definition`. The runtime is
//! intentionally identical (libc malloc/free, abort-on-panic) so it
//! does not matter which copy wins.
//!
//! Only compiled for the `target_env = "musl"` staticlib build — host
//! tests and rlib consumers stay on the regular Rust std / testing
//! alloc / panic infrastructure.

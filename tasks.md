...
| Track | Scope | Status |
|---|---|---|
| A | ELF parser + memory (`mm/elf.rs`) | 🔄 in progress |
| B | Process table (`process/`) | 🔄 in progress |
| C | ABI + syscalls | ⏳ blocked on A+B |
| D | Init integration + validation | ⏳ blocked on C |
| E | Documentation | ⏳ blocked on C |

...
The text the user typed after `/sdd-specify` in the triggering message **is** the feature description. Look at the user's full message to find it. Do not ask the user to repeat it unless they provided an empty command.
...
4. Write the specification to `.sdd/{FEATURE_SHORT_NAME}/{SPEC_FILE}` using the template structure, replacing placeholders with concrete details derived from the feature description while preserving section order and headings.

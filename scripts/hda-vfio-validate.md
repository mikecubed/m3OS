...
> **Known limitation — codec enumeration was not reached under this VFIO config.**
...
> `STATESTS` stays `0x0000`. The **leading hypothesis** is codec-rail power/clock
> gating: when only the audio function (`10:00.6`) is passed through, the guest
> never replays the platform ACPI `_PS0` power resources that gate the codec link
> clock. This is **inferred by elimination, not proven**, and untried passthrough
> workarounds remain (host-powered handoff, `vfio-pci.disable_idle_d3=1`, a
> `probe_mask`-style forced probe — see the capture doc). So VFIO here confirms
> **controller bring-up + config-space write**, not codec enumeration or audible
> output; the latter is validated only on **bare metal** (the safe path). Run the
> steps below to reproduce the controller-side confirmation, not as a path to
> audible sound.
